...
public class MSELossKernel extends BaseKernel {
    ...
    private CUfunction loss_right_function;
    private CUfunction loss_right_backward_function;
    ...
    public void initFunction() {
        try {
            ...
            if (loss_right_function == null) {
                loss_right_function = getCudaManager().getLocalFunctionByModule("MSELossKernel.cu", "loss_right");
            }
            if (loss_right_backward_function == null) {
                loss_right_backward_function = getCudaManager().getLocalFunctionByModule("MSELossKernel.cu", "loss_right_back");
            }
        } catch (Exception e) {
    ...
    public void forward_right(Tensor input, Tensor currentLabel, Tensor output) {
        ...
        loss_kernelParameters = Pointer.to(Pointer.to(input.getGpuData()), Pointer.to(currentLabel.getGpuData()), Pointer.to(output.getGpuData()), Pointer.to(new int[]{input.number}), Pointer.to(new int[]{input.channel * input.height * input.width}));
        this.N = output.number;
        cuLaunchKernel(loss_right_function, input.number, 1, 1,      // Grid dimension
                CAFFE_CUDA_NUM_THREADS, 1, 1,      // Block dimension
                0, null,               // Shared memory size and stream
                loss_kernelParameters, null // Kernel- and extra parameters
        );
