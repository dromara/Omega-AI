...
import com.omega.engine.nn.layer.va_vae.VAVAEDecoder;
import com.omega.engine.nn.layer.va_vae.VAVAEEncoder;
 ...
 */
public class VA_VAE extends Network {
    ...
    public VAVAEEncoder encoder;
    public VAVAEDecoder decoder;
    ...
    public VA_VAE(LossType lossType, UpdaterType updater,int latendDim, int imageSize, int[] ch_mult, int ch, int num_res_blocks, boolean double_z) {
    ...
    public void initLayers() {
        ...
        if(double_z) {
        	this.encoder = new VAVAEEncoder(3, latendDim * 2, imageSize, imageSize, num_res_blocks, groups, ch_mult, ch, true, this);
        	pre_quant_conv = new ConvolutionLayer(latendDim * 2, latendDim * 2, encoder.oWidth, encoder.oHeight, 1, 1, 0, 1, true, this);
        }else {
        	this.encoder = new VAVAEEncoder(3, latendDim, imageSize, imageSize, num_res_blocks, groups, ch_mult, ch, true, this);
        ...
        this.decoder = new VAVAEDecoder(latendDim, 3, encoder.oHeight, encoder.oWidth, num_res_blocks, groups, ch_mult, ch, true, this);
