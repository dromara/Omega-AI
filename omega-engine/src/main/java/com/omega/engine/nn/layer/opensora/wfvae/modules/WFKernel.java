...
//        float[] bias1 = MatrixUtils.one(x1.length);
    ...
    public void append(Tensor input, Tensor output, int length, int count, int currentIndex) {
        try {
            /**
             * 设置入参
             *
             */
            forwardKernelParameters = Pointer.to(Pointer.to(new int[]{input.dataLength}),Pointer.to(input.getGpuData()),
                    Pointer.to(output.getGpuData()),
                    Pointer.to(new int[]{length}), Pointer.to(new int[]{count}), Pointer.to(new int[]{currentIndex}));

            cuLaunchKernel(function_append, this.CAFFE_GET_BLOCKS(input.dataLength), 1, 1,      // Grid dimension
                    CAFFE_CUDA_NUM_THREADS, 1, 1,      // Block dimension
                    0, null,               // Shared memory size and stream
                    forwardKernelParameters, null // Kernel- and extra parameters
            );
