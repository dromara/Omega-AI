 ...
 */
public class DiTAttentionLayer extends Layer {
    ...
    public DiTAttentionLayer(int embedDim, int headNum, int time, boolean bias, boolean qkNorm) {
        this.bias = bias;
        this.time = time;
        this.qkNorm = qkNorm;
    ...
    public DiTAttentionLayer(int embedDim, int headNum, int time, boolean bias, boolean qkNorm, Network network) {
        this.bias = bias;
        this.qkNorm = qkNorm;
    ...
    public static void main(String[] args) {
        ...
        DiTAttentionLayer mal = new DiTAttentionLayer(embedDim, headNum, time, false, false, tf);
        for (int i = 0; i < 10; i++) {
            //			input.showDM();
            mal.forward(input);
    ...
    public void scaledDotProductAttentionBackward() {
        ...
        if(qkNorm) {
        	// backward into q
            GPUOP.getInstance().bmmEX(CUBLAS_OP_N, CUBLAS_OP_N, dk, time, time, 1.0f, kNorm.getOutput().getGpuData(), dk, time * dk, dpreatt.getGpuData(), time, time * time, 0.0f, dqt.getGpuData(), dk, time * dk, batchSize * headNum);
            // backward into k
            GPUOP.getInstance().bmmEX(CUBLAS_OP_N, CUBLAS_OP_T, dk, time, time, 1.0f, qNorm.getOutput().getGpuData(), dk, time * dk, dpreatt.getGpuData(), time, time * time, 0.0f, dkt.getGpuData(), dk, time * dk, batchSize * headNum);
        }else {
        	// backward into q
    ...
    public void diff() {
        ...
        if(qkNorm) {
        	qNorm.back(dqt);
        	kNorm.back(dkt);
        	attentionKernel.permute_backward(dqkv, qNorm.diff, kNorm.diff, dvt, batchSize, time, headNum, dk);
        }else {
