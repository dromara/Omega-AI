...
import com.omega.engine.nn.layer.dit.DiTLabelEmbedder;
 ...
 */
public class DiTMainMoudue extends Layer {
    ...
    public DiTLabelEmbedder labelEmbd;
    ...
    public DiTMainMoudue(int inChannel, int width, int height, int patchSize, int hiddenSize, int headNum, int depth, int timeSteps, int classNum, int mlpRatio, boolean learnSigma, float y_drop_prob, Network network) {
...
//		this.bias = bias;
    ...
    public void initLayers() {
        ...
        labelEmbd = new DiTLabelEmbedder(classNum, hiddenSize, y_drop_prob, network);
        
        blocks = new ArrayList<DiTBlock>();
         
        for(int i = 0;i<depth;i++) {
        	DiTBlock block = new DiTBlock(hiddenSize, hiddenSize, patchEmbd.oChannel, mlpRatio * hiddenSize, headNum, true, false, network);
	        blocks.add(block);
        }
        int os = inChannel;
        if(learnSigma) {
...
//    	System.err.println("emb:"+JsonUtils.toJson(emb));
    ...
    public void output(Tensor tc,Tensor label) {
    	...
    	Tensor_OP().add(timeEmbd.getOutput(), labelEmbd.getOutput(), labelEmbd.getOutput());
    	
    	for(int i = 0;i<depth;i++) {
    		DiTBlock block = blocks.get(i);
    		block.forward(x, labelEmbd.getOutput());
    		x = block.getOutput();
    	}

    	finalLayer.forward(x, labelEmbd.getOutput());
    ...
    public void output(Tensor tc,Tensor label,Tensor cos,Tensor sin) {
    	...
    	for(int i = 0;i<depth;i++) {
    		DiTBlock block = blocks.get(i);
    		block.forward(x, labelEmbd.getOutput(), cos, sin);
    		x = block.getOutput();
...
//     	dy.showDM("block-diff");
    ...
    public static void loadWeight(Map<String, Object> weightMap, DiTMainMoudue block, boolean showLayers) {
        if (showLayers) {
