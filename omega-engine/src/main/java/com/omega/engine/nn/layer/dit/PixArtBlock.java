...
public class PixArtBlock extends Layer {
    ...
    public void output(Tensor tc,Tensor text) {
    	...
    	cross_attn.forward(crossAttnInput, text);
    ...
    public void output_eval(Tensor tc,Tensor text,Tensor cos,Tensor sin) {
    	 ...
    	 */
    	cross_attn.forward(x, text, cos , sin);
    ...
    public void diff(Tensor dtc,Tensor dText) {
    	...
    	Tensor_OP().add(cross_attn.diff, norm3.diff, cross_attn.diff);
    	
    	Tensor_OP().mul_left_back(modulation_gate_msa.getOutput(), cross_attn.diff, output,  batchSize, time, 1, output.width, 1);
    	attn.back(output);
    	Tensor_OP().mul_right_back(attn.getOutput(), cross_attn.diff, modulation_gate_msa.getOutput(), batchSize, time, 1, output.width, 1);
    	...
    	Tensor_OP().add(norm1.diff, cross_attn.diff, norm1.diff);
