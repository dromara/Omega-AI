 ...
 */
public class PixArtMoudue extends Layer {
    ...
    public List<PixArtBlock> blocks;
    ...
    public PixArtMoudue(int inChannel, int width, int height, int patchSize, int hiddenSize, int headNum, int depth, int timeSteps, int maxContextLen, int textEmbedDim, int mlpRatio, boolean learnSigma, float y_drop_prob, Network network) {
...
//		this.bias = bias;
    ...
    public void initLayers() {
        ...
        blocks = new ArrayList<PixArtBlock>();
         
        for(int i = 0;i<depth;i++) {
        	PixArtBlock block = new PixArtBlock(hiddenSize, hiddenSize, hiddenSize, patchEmbd.oChannel, maxContextLen, mlpRatio * hiddenSize, headNum, true, false, network);
	        blocks.add(block);
        }
        int os = inChannel;
        if(learnSigma) {
...
//    	System.err.println("emb:"+JsonUtils.toJson(emb));
    ...
    public void output(Tensor tc,Tensor text) {
    	...
    	for(int i = 0;i<depth;i++) {
    		PixArtBlock block = blocks.get(i);
...
//     	dy.showDM("block-diff");
    ...
    public static void loadWeight(Map<String, Object> weightMap, PixArtMoudue block, boolean showLayers) {
        if (showLayers) {
