...
public class GANOptimizer extends Optimizer {
    ...
    public void trainDG(BaseData trainingData, int index, int[][] indexs, int it, Tensor true_labels, Tensor fake_labels, Tensor inputG, Tensor inputD, Graph gd1, Graph gd2, Graph gg, Tensor d_fake_output, Tensor d_true_output) {
        Tensor g_fake_output = null;
        if (it % d_every == 0) {
            trainingData.getRandomData(indexs[it], inputD);
            inputD.hostToDevice();
    ...
    public float trainDG(ImageDataLoader trainingData, int index, int[] indexs, int it, Tensor true_labels, Tensor fake_labels, Tensor inputG, Tensor inputD, Graph gd1, Graph gd2, Graph gg, Tensor d_fake_output, Tensor d_true_output, float d_loss_current) {
        Tensor g_fake_output = null;
        gd1.start();
        gd2.start();
        if (it % d_every == 0) {
    ...
    public float trainDG(ImageDataLoader trainingData, int index, int[] indexs, int it, Tensor true_labels, Tensor fake_labels, Tensor inputG, Tensor inputD, Tensor d_fake_output, Tensor d_true_output, BCELoss lossD, BCELoss lossG, float d_loss_current) {
        /**
         * 生成器生成假图片
     ...
     * 训练判别器
     */
    public void trainNetD(ImageDataLoader trainingData, int[][] indexs, int it, Tensor true_labels, Tensor fake_labels, Tensor inputG, Tensor inputD, Tensor d_real_loss, Tensor d_fake_loss, Tensor d_real_diff, Tensor d_fake_diff, Tensor diff) {
