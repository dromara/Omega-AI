 ...
 * @diff: 2 * (y - f(x))
 */
public class MSELoss_right extends LossFunction {
    private static MSELoss_right instance;
    public final LossType lossType = LossType.MSE;
    private MSELossKernel kernel;
    private Tensor loss;
    private Tensor diff;

    public MSELoss_right(Network network) {
        setNet(network);
        kernel = new MSELossKernel(network.cudaManager);
    }

    public MSELoss_right(CUDAManager cudaManager) {
        kernel = new MSELossKernel(cudaManager);
    }

    public static MSELoss_right operation(CUDAManager cudaManager) {
        if (instance == null) {
            instance = new MSELoss_right(cudaManager);
        }
        return instance;
    }
    ...
    public static void main(String[] args) {
        ...
        Tensor loss = MSELoss_right.operation(cudaManager).loss(xt, labelt);
        loss.showDM();
        Tensor diff = MSELoss_right.operation(cudaManager).diff(xt, labelt);
        diff.showDM();
    }
    ...
    public Tensor loss(Tensor x, Tensor label) {
        ...
        kernel.forward_right(x, label, loss);
    ...
    public Tensor diff(Tensor x, Tensor label) {
        // TODO Auto-generated method stub
        kernel.backward_right(x, label, diff);
        return diff;
    }
