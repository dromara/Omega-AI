 ...
 */
public class FluxDiTBlockFullTime extends Layer {
...
//        	shift_msa = Tensor.createGPUTensor(this.shift_msa, number, 1, 1, embedDim, true);
//        	scale_msa = Tensor.createGPUTensor(this.scale_msa, number, 1, 1, embedDim, true);
//        	gate_msa = Tensor.createGPUTensor(this.gate_msa, number, 1, 1, embedDim, true);
//    		shift_mlp = Tensor.createGPUTensor(this.shift_mlp, number, 1, 1, embedDim, true);
//        	scale_mlp = Tensor.createGPUTensor(this.scale_mlp, number, 1, 1, embedDim, true);
//        	gate_mlp = Tensor.createGPUTensor(this.gate_mlp, number, 1, 1, embedDim, true);
        }
...
//    	attn.getOutput().showDM("attn");
    ...
    public void modulate_back(Tensor dShift, Tensor dScale, Tensor dx, Tensor x, Tensor scale, Tensor delta) {
    	Tensor_OP().copyGPU(delta, dShift);
    ...
    public void diff(Tensor dtc) {
    	...
    	mlp.back(output);
    	Tensor_OP().mul(mlp.getOutput(), delta, gate_mlp);
    	...
    	Tensor_OP().mul(gate_msa, norm3.diff, output);
    	attn.back(output);
    	Tensor_OP().mul(attn.getOutput(), norm3.diff, gate_msa);
...
//    	attn.diff.showDM("attn.diff");
    ...
    public static void loadWeight(Map<String, Object> weightMap, FluxDiTBlockFullTime block, boolean showLayers) {
        if (showLayers) {
    ...
    public static void main(String[] args) {
        ...
        FluxDiTBlockFullTime block = new FluxDiTBlockFullTime(embedDim, embedDim, time, embedDim * 4, headNum, 77, true, false, tf);
