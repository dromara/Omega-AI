 ...
 */
public class OmegaVideoDiTMainI2V2 extends Layer {
    ...
    public OmegaVideoDiTMainI2V2(int inChannel, int num_frames, int height, int width, int patchSize, int hiddenSize, int headNum, int depth, int timeSteps, float token_drop_ratio, float path_drop_prob, Network network) {
    ...
    public void initLayers() {
    	...
    	patchEmbd = new OmegaVideoPatchEmbed(inChannel * 2, num_frames, height, width, hiddenSize, patchSize, true, network);
    ...
    public void init(Tensor input) {
        ...
        if(cat_x == null || cat_x.number != number) {
        	cat_x = Tensor.createGPUTensor(cat_x, number, inChannel * 2 * num_frames, height, width, true);
        }
    ...
    public void output(Tensor z_img, Tensor tc, Tensor[] cos, Tensor[] sin) {
    	
    	baseKernel.concat_channel_forward(input, z_img, cat_x, input.number, inChannel, inChannel, 1, num_frames * height * width);
    	
    	patchEmbd.forward(cat_x);
...
//     	de.showDMByOffsetRed(150 * 1152, 1152, "img_x");
     ...
     * @param text
     */
    public void forward(Tensor input, Tensor z_img, Tensor tc, Tensor[] cos, Tensor[] sin) {
        // TODO Auto-generated method stub
        /**
         * 设置输入
         */
    ...
    public static void loadWeight(Map<String, Object> weightMap, OmegaVideoDiTMainI2V2 block, boolean showLayers) {
        if (showLayers) {
...
//    	
//        OmegaVideoDiTMainI2V2 jb = new OmegaVideoDiTMainI2V2(C, num_frames, H, W, patchSize, hiddenSize, headNum, depth, 1000, 0.0f, 0.05f, nn);
//    	
