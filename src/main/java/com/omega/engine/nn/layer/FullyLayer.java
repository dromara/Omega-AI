...
public class FullyLayer extends Layer {
    ...
    //	private cudaStream_t defaultStream;
    //
    ...
    public void initKernel() {
        ...
        //
        //			defaultStream = new cudaStream_t();
        //
        //			dwStream = new cudaStream_t();
        //
        //			dxStream = new cudaStream_t();
        //
        //			JCuda.cudaStreamCreate(dwStream);
        //			JCuda.cudaStreamCreate(dxStream);
        //
        //		}
    }
    ...
    public void output(int batch, int step) {
        // TODO Auto-generated method stub
        if (this.input != null) {
            GPU_OP().multiplyFloat(batch, oWidth, width, input.getGpuData().withByteOffset(step * batch * input.getOnceSize() * Sizeof.FLOAT), weight.getGpuData(), output.getGpuData().withByteOffset(step * batch * output.getOnceSize() * Sizeof.FLOAT), cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_N, 1.0f, 0.0f);
            if (hasBias) {
    ...
    public void output(int batch, int inputStep, int step) {
        ...
        if (this.input != null) {
            if (inputStep >= 0) {
                GPU_OP().multiplyFloat(batch, oWidth, width, input.getGpuData().withByteOffset(inputStep * batch * input.getOnceSize() * Sizeof.FLOAT), weight.getGpuData(), output.getGpuData().withByteOffset(step * batch * output.getOnceSize() * Sizeof.FLOAT), cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_N, 1.0f, 0.0f);
            }
            if (hasBias) {
                kernel.addBias(output, bias, batch, step);
            }
    ...
    public void diff(int batch, int step) {
         ...
         */
        GPU_OP().multiplyFloat(this.width, this.oWidth, batch, input.getGpuData().withByteOffset(step * batch * input.getOnceSize() * Sizeof.FLOAT), delta.getGpuData().withByteOffset(step * batch * delta.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(), cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
        if (hasBias) {
         ...
         */
        GPU_OP().multiplyFloat(batch, this.width, this.oWidth, delta.getGpuData().withByteOffset(step * batch * delta.getOnceSize() * Sizeof.FLOAT), weight.getGpuData(), diff.getGpuData().withByteOffset(step * batch * diff.getOnceSize() * Sizeof.FLOAT), cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_T, 1.0f, 0.0f);
    }
    ...
    public void diff(int batch, int inputStep, int step) {
         ...
         */
        GPU_OP().multiplyFloat(this.width, this.oWidth, batch, input.getGpuData().withByteOffset(inputStep * batch * input.getOnceSize() * Sizeof.FLOAT), delta.getGpuData().withByteOffset(step * batch * delta.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(), cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
        if (hasBias) {
            kernel.backwardBias(diffB, delta, batch, step);
        }
        /**
    ...
    public void diff(int batch, int inputStep, int deltaStep, int step) {
        ...
        if (step >= 0) {
             ...
             */
            GPU_OP().multiplyFloat(this.width, this.oWidth, batch, input.getGpuData().withByteOffset(step * batch * input.getOnceSize() * Sizeof.FLOAT), delta.getGpuData().withByteOffset(inputStep * batch * delta.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(), cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
            if (hasBias) {
             ...
             */
            GPU_OP().multiplyFloat(batch, this.width, this.oWidth, delta.getGpuData().withByteOffset(inputStep * batch * delta.getOnceSize() * Sizeof.FLOAT), weight.getGpuData(), diff.getGpuData().withByteOffset(step * batch * diff.getOnceSize() * Sizeof.FLOAT), cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_T, 1.0f, 0.0f);
        }
    }
    ...
    public void diff(Tensor input, Tensor diff, int batch, int deltaStep) {
         ...
         */
        GPU_OP().multiplyFloat(this.width, this.oWidth, batch, input.getGpuData(), delta.getGpuData().withByteOffset(deltaStep * batch * delta.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(), cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
        if (hasBias) {
         ...
         */
        GPU_OP().multiplyFloat(batch, this.width, this.oWidth, delta.getGpuData().withByteOffset(deltaStep * batch * delta.getOnceSize() * Sizeof.FLOAT), weight.getGpuData(), diff.getGpuData(), cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_T, 1.0f, 0.0f);
    }
    ...
    public void diffTemp() {
         ...
         */
        GPU_OP().multiplyFloat(this.width, this.oWidth, this.number, input.getGpuData(), delta.getGpuData(), diffW.getGpuData(), cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 0.5f, 1.0f);
        if (hasBias) {
            kernel.backwardBias(diffB, delta);
        }
        /**
         ...
         */
        GPU_OP().multiplyFloat(this.number, this.width, this.oWidth, delta.getGpuData(), weight.getGpuData(), diff.getGpuData(), cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_T, 1.0f, 1.0f);
    }
