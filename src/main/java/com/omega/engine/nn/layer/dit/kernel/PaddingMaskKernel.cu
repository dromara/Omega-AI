...
extern "C"
__global__ void set_mask_back_igone(const size_t size, float *dout, const int T, const int igoneT, const int W)
{
...
extern "C"
__global__ void mask_igone_diff(
    const float* __restrict__ inp,  // 输入：(M, N) 行优先存储
    float* __restrict__ out,        // 输出：(N,)
    int M,                      // 行数
    int N,                       // 列数
