...
//    public LNLayer norm1;
    ...
    public DiTBlock(int embedDim, int cEmbedDim, int time, int mlpHiddenDim, int headNum, boolean bias, boolean qkNorm) {
    ...
    public DiTBlock(int embedDim, int cEmbedDim, int time, int mlpHiddenDim, int headNum, boolean bias, boolean qkNorm, Network network) {
    ...
    public void output(Tensor tc) {
    	...
    	modulate(norm2.getOutput(), modulation_shift_mlp.getOutput(), modulation_scale_mlp.getOutput(), mlpInput);
    ...
    public void output(Tensor tc,Tensor cos,Tensor sin) {
    	...
    	norm2.forward(crossAttnInput);
    	/**
    	 * x2 = x1 + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm2(x1), shift_mlp, scale_mlp))
    	 */
    	modulate(norm2.getOutput(), modulation_shift_mlp.getOutput(), modulation_scale_mlp.getOutput(), mlpInput);
    	mlp.forward(mlpInput);
    	Tensor_OP().mul(mlp.getOutput(), modulation_gate_mlp.getOutput(), output, batchSize, time, 1, output.width, 1);
