...
public class DiTBlock extends Layer {
    ...
    public DiTBlock(int embedDim, int cEmbedDim, int time, int mlpHiddenDim, int headNum, int maxContext, boolean bias, boolean qkNorm, Network network) {
    ...
    public void output(Tensor tc, Tensor context,Tensor cos,Tensor sin) {
    	...
    	norm1.forward(input);
    	modulate(norm1.getOutput(), shift_msa, scale_msa, attnInput);
    	attn.forward(attnInput, context, cos , sin);
//    	attn.getOutput().showDM("attn");
    ...
    public void output(Tensor tc, Tensor context, Tensor cos, Tensor sin, Tensor idskeep) {
    	...
    	norm1.forward(input);
    	modulate(norm1.getOutput(), shift_msa, scale_msa, attnInput);
    	attn.forward(attnInput, context, cos , sin, idskeep);
//    	attn.getOutput().showDM("attn");
    	Tensor_OP().mul(attn.getOutput(), gate_msa, crossAttnInput, batchSize, time, 1, crossAttnInput.width, 1);
    	Tensor_OP().add(input, crossAttnInput, crossAttnInput);
    ...
    public void diff(Tensor dtc, Tensor dcontext, Tensor cos, Tensor sin) {
//    	delta.showDM("x3");
    	/**
...
//    	mlp.diff.showDM("mlp");
    	...
    	attn.back(output, dcontext, cos, sin);
...
//    	output.showDM("attn_delta");
    	...
    	attn.back(output, dcontext, cos, sin, idskeep);
...
//    	attn.diff.showDM("attn.diff");
     ...
     * @param text
     */
    public void forward(Tensor input,Tensor tc, Tensor context,Tensor cos,Tensor sin) {
        // TODO Auto-generated method stub
        /**
         * 设置输入
         */
         ...
         * 计算输出
         */
        this.output(tc, context, cos, sin);
    }
    
    public void forward(Tensor input,Tensor tc, Tensor context,Tensor cos,Tensor sin, Tensor idskeep) {
        // TODO Auto-generated method stub
        /**
         * 设置输入
         */
         ...
         * 计算输出
         */
        this.output(tc, context, cos, sin, idskeep);
    }
    ...
    public void back(Tensor delta,Tensor dtc, Tensor dcontext,Tensor cos,Tensor sin) {
        // TODO Auto-generated method stub
        this.initBack();
        /**
         * 设置梯度
         ...
         * 计算梯度
         */
        this.diff(dtc, dcontext, cos, sin);
        if (this.network.GRADIENT_CHECK) {
            this.gradientCheck();
        }
    }
    
    public void back(Tensor delta,Tensor dtc, Tensor dcontext,Tensor cos,Tensor sin, Tensor idskeep) {
        // TODO Auto-generated method stub
        this.initBack();
        /**
         * 设置梯度
         ...
         * 计算梯度
         */
        this.diff(dtc, dcontext, idskeep, cos, sin);
        if (this.network.GRADIENT_CHECK) {
            this.gradientCheck();
        }
    }
