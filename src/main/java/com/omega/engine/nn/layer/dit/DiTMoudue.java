...
public class DiTMoudue extends Layer {
    ...
    public DiTMoudue(int inChannel, int width, int height, int patchSize, int hiddenSize, int headNum, int depth, int timeSteps, int maxContextLen, int textEmbedDim, int mlpRatio,  Network network) {
...
//		this.bias = bias;
    ...
    public void initLayers() {
        ...
        for(int i = 0;i<depth;i++) {
	        DiTBlock block = new DiTBlock(hiddenSize, hiddenSize, textEmbedDim, patchEmbd.oChannel, maxContextLen, mlpRatio * hiddenSize, headNum, true, network);
	        blocks.add(block);
        }
         
        finalLayer = new DiTFinalLayer(patchSize, hiddenSize, inChannel, patchEmbd.oChannel, true, network);
...
//    	System.err.println("emb:"+JsonUtils.toJson(emb));
    ...
    public void init(Tensor input) {
        ...
        if(this.output == null || this.output.number != number) {
        	output = Tensor.createGPUTensor(output, input.shape(), true);
        }
        if(posEmbd == null) {
    ...
    public void initBack() {
    	...
    	if(dtc == null || dtc.number != timeEmbd.getOutput().number) {
    		...
    		dtext = Tensor.createGPUTensor(dtext, timeEmbd.getOutput().number * maxContextLen, 1, 1, textEmbedDim, true);
    	}else {
    		dtc.clearGPU();
    	}
    }
    ...
    public void output(Tensor tc,Tensor text) {
    	...
    	for(int i = 0;i<depth;i++) {
    		...
    		block.forward(x, timeEmbd.getOutput(), text);
    		x = block.getOutput();
    	}
    	...
    	int[] xShape = new int[] {number, h, w, patchSize, patchSize, inChannel};
    	int[] yShape = new int[] {number, inChannel, h, patchSize, w, patchSize};
    ...
    public void output(Tensor tc,Tensor text,Tensor cos,Tensor sin) {
    	...
    	patchEmbd.forward(input);
//    	Tensor_OP().add(patchEmbd.getOutput(), posEmbd, patchEmbd.getOutput());
    	
    	timeEmbd.forward(tc);
//    	patchEmbd.getOutput().showDM("patchEmbd");
//    	timeEmbd.getOutput().showDM("time_out");
    	Tensor x = patchEmbd.getOutput().view(patchEmbd.getOutput().number * patchEmbd.getOutput().channel, 1, 1, patchEmbd.getOutput().width);

    	for(int i = 0;i<depth;i++) {
    		DiTBlock block = blocks.get(i);
    		block.forward(x, timeEmbd.getOutput(), text, cos, sin);
    		x = block.getOutput();
    	}
