...
import com.omega.example.unet.utils.SegImageLoader2;
 ...
 */
public class ImageClip_REAP_DataLoader extends BaseDataLoader {
    ...
    public ImageClip_REAP_DataLoader(String labelPath, String imgDirPath, String imgDirPath2, String clipDataPath, String extName, int img_w, int img_h, int img2_w, int img2_h, int maxContextLen, int yDim, int batchSize, boolean horizontalFilp, float[] mean, float[] std, float[] mean2, float[] std2) {
        this.horizontalFilp = horizontalFilp;
        this.imgDirPath = imgDirPath;
        this.imgDirPath2 = imgDirPath2;
    ...
    public void loadData(int[] indexs, Tensor input, Tensor input2, Tensor label) {
        // TODO Auto-generated method stub
    	try {
            if (cf != null) {
                boolean success = cf.get();
                if(success){
                	cf = null;
                	loadDataToGPU(indexs, input, input2, label);
                }
            } else {
                cf = loadAsyncData(indexs, input, input2, label);
                boolean success = cf.get();
                if(success){
                	cf = null;
                	loadDataToGPU(indexs, input, input2, label);
    ...
    public void loadDataToGPU(int[] index,Tensor input, Tensor input2, Tensor label) {
    	input.hostToDevice();
    	input2.hostToDevice();
        label.hostToDevice();
        cf = loadAsyncData(index, input, input2, label);
    }
    ...
    public CompletableFuture<Boolean> loadAsyncData(int[] indexs,Tensor input, Tensor input2, Tensor label) {
        CompletableFuture<Boolean> cf = CompletableFuture.supplyAsync(() -> {
            try {
            	/**
                 * 加载input数据
                 */
                if (mean != null) {
                    SegImageLoader2.load(imgDirPath, imgDirPath2, extName, idxSet, indexs, input.number, input, input2, false, true, mean, std, mean2, std2);
                } else {
                    SegImageLoader2.load(imgDirPath, imgDirPath2, extName, idxSet, indexs, input.number, input, input2, false, true);
                }
                loadLabels(indexs, label);
            } catch (Exception e) {
                // TODO: handle exception
