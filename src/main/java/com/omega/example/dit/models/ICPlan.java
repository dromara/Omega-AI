...
//			f0.showDM("f0");
	...
	public Tensor forward_with_path_drop_cfg_heun_step(OmegaDiT_Self_Flow dit, Tensor y0, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor eps, Tensor eps2, float cfg_scale) {
		ininT(0, 1, count);
		Tensor out = null;
		for(int i = 0;i<count - 2;i++) {
			float t0 = T[i];
	...
	public Tensor euler_step(OmegaDiT_Self_Flow dit, Tensor z, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor eps, float t0, float t1, float cfg_scale) {
		float t_ = t1 - t0;
		Tensor v_pred = dit.forward_with_path_drop_cfg(this, z, t, context, null_context, cos, sin, eps, cfg_scale);
		dit.tensorOP.mul(v_pred, t_, v_pred);
		dit.tensorOP.add(z, v_pred, v_pred);
		return v_pred;
	}
	
	public Tensor heun_step(OmegaDiT_Self_Flow dit, Tensor z, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor eps, Tensor eps2, float t0, float t1, float cfg_scale) {
		Tensor v_pred_t = dit.forward_with_path_drop_cfg(this, z, t, context, null_context, cos, sin, eps, cfg_scale);
	...
	public Tensor euler_step(OmegaDiT dit, Tensor z, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor eps, float t0, float t1, float cfg_scale) {
		float t_ = t1 - t0;
		Tensor v_pred = dit.forward_with_path_drop_cfg(this, z, t, context, null_context, cos, sin, eps, cfg_scale);
		dit.tensorOP.mul(v_pred, t_, v_pred);
		dit.tensorOP.add(z, v_pred, v_pred);
	...
	public Tensor heun_step(OmegaDiT dit, Tensor z, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor eps, Tensor eps2, float t0, float t1, float cfg_scale) {
		Tensor v_pred_t = dit.forward_with_path_drop_cfg(this, z, t, context, null_context, cos, sin, eps, cfg_scale);
	...
	public Tensor euler_step(OmegaDiT2 dit, Tensor z, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor eps, float t0, float t1, float cfg_scale) {
		float t_ = t1 - t0;
		Tensor v_pred = dit.forward_with_path_drop_cfg(this, z, t, context, null_context, cos, sin, eps, cfg_scale);
		dit.tensorOP.mul(v_pred, t_, v_pred);
		dit.tensorOP.add(z, v_pred, v_pred);
	...
	public Tensor heun_step(OmegaDiT2 dit, Tensor z, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor eps, Tensor eps2, float t0, float t1, float cfg_scale) {
		Tensor v_pred_t = dit.forward_with_path_drop_cfg(this, z, t, context, null_context, cos, sin, eps, cfg_scale);
	...
	public Tensor forward_with_path_drop_cfg(OmegaVideo dit, Tensor y0, Tensor t, Tensor context, Tensor null_context, Tensor[] cos, Tensor[] sin, Tensor y1, Tensor eps, float cfg_scale) {
		ininT(0, 1, count);
		int j = 1;
		Tensor out = null;
		Tensor f0 = null;
	...
	public Tensor forward_with_path_drop_cfg(OmegaVideo2 dit, Tensor y0, Tensor t, Tensor context, Tensor null_context, Tensor[] cos, Tensor[] sin, Tensor y1, Tensor eps, float cfg_scale) {
		ininT(0, 1, count);
		int j = 1;
		Tensor out = null;
		Tensor f0 = null;
	...
	public Tensor forward_with_path_drop_cfg(OmegaVideoI2V dit, int[] shape, Tensor y0, Tensor imgs, Tensor t, Tensor[] cos, Tensor[] sin, Tensor y1, Tensor eps, float cfg_scale) {
		...
		Tensor out = null;
		Tensor f0 = null;
//		System.err.println(JsonUtils.toJson(new int[] {shape[0] * shape[1], shape[2], shape[3], shape[4]}));
		...
		for(int i = 0;i<count - 1;i++) {
			...
			MatrixUtils.val(t.data, t0);
			t.hostToDevice();
			f0 = dit.forward_with_path_drop_cfg(y0, t, cos, sin, eps, cfg_scale);
			dit.tensorOP.mul(f0, dt, f0);
			dit.tensorOP.add(y0, f0, y1);
			float tj = T[j];
			if(j < T.length && t1 >= tj) {
...
//			f0.showDM("f0");
	...
	public Tensor forward_with_cfg(MMJiT_REPA jit, Tensor x, Tensor t, Tensor context, Tensor null_context, Tensor cos1d, Tensor sin1d, Tensor cos2d, Tensor sin2d, Tensor eps, float cfg_scale) {
		ininT(0, 1, count+1);
		Tensor v = null;
		for(int i = 0;i<count;i++) {
			float t0 = T[i];
	...
	public Tensor forward_with_path_drop_cfg(MMJiT_Sprint dit, Tensor y0, Tensor t, Tensor context, Tensor null_context, Tensor cos1d, Tensor sin1d, Tensor cos2d, Tensor sin2d, Tensor y1, Tensor eps, float cfg_scale) {
		ininT(0, 1, count);
		int j = 1;
		Tensor out = null;
		Tensor f0 = null;
	...
	public Tensor forward_with_path_drop_cfg(JiT_Sprint2 dit, Tensor y0, Tensor t, Tensor context, Tensor null_context, Tensor cos, Tensor sin, Tensor y1, Tensor eps, float cfg_scale) {
		ininT(0, 1, count);
		int j = 1;
		Tensor out = null;
		Tensor f0 = null;
		for(int i = 0;i<count - 1;i++) {
			...
			MatrixUtils.val(t.data, t0);
			t.hostToDevice();
			f0 = dit.forward_with_path_drop_cfg(this, y0, t, context, null_context, cos, sin, eps, cfg_scale);
			dit.tensorOP.mul(f0, dt, f0);
			dit.tensorOP.add(y0, f0, y1);
			float tj = T[j];
			if(j < T.length && t1 >= tj) {
	...
	public Tensor forward_with_path_drop_cfg(JiT_Sprint dit, Tensor y0, Tensor t, Tensor context, Tensor null_context, Tensor cos1d, Tensor sin1d, Tensor cos2d, Tensor sin2d, Tensor y1, Tensor eps, float cfg_scale) {
		ininT(0, 1, count);
		int j = 1;
		Tensor out = null;
		Tensor f0 = null;
...
//		op.add(x2, dx1, dx1);
	...
	public void expand_mask_skip_text(Tensor a, Tensor b, Tensor mask, Tensor out, int W, int textTokenCount, float maskRatio) {
		kernel.expand_mask_skip_text(a, b, mask, out, W, textTokenCount, maskRatio);
	}
