...
/**
 * DCAE_ResidualUpsampleBlock
 *
 * @author Administrator
 */
public class DCAEResidualUpsampleBlock extends Layer {
    ...
    public DCAEResidualUpsampleBlock(int channel, int oChannel, int height, int width, int group, Network network) {
    ...
    public void initLayers(int oChannel) {
    	...
    	repeats = oChannel / (channel / 4);
    	
    	up = new UPSampleLayer2(channel, height, width, 2, network);
    	
    	conv1 = new ConvolutionLayer(channel, oChannel, up.oWidth, up.oHeight, 3, 3, 1, 1, true, this.network);
...
//        }
    ...
    public void init(Tensor input) {
        ...
        if(sc_x == null || sc_x.getShape()[0] != number) {
    		x_shape = new int[] {number, channel / 4, 2, 2, height, width};
    		o_shape = new int[] {number, channel / 4, height, 2, width, 2};
        	sc_x = Tensor.createGPUTensor(sc_x, number, channel / 4, height * 2, width * 2, true);
        }
        if(ca_x == null || ca_x.getShape()[0] != number) {
        	ca_x = Tensor.createGPUTensor(ca_x, number, this.oChannel, oHeight, oWidth, true);
        }
...
//        } else {
    		 ...
    		 * space_to_channel
    		 */
    		Tensor_OP().permute(input, sc_x, x_shape, o_shape, new int[] {0, 1, 4, 2, 5, 3});
    		
    		/**
    		 * channel_average
    		 */
    		kernel.channel_duplicate(sc_x, ca_x, repeats);
    		
    		up.forward(input);
    		
        	conv1.forward(up.getOutput());
...
//    }
    ...
    public void diff() {
        ...
        kernel.channel_duplicate_backward(delta, sc_x, repeats);
        ...
        a1.back(conv2.diff);
        conv1.back(a1.diff, conv1.input);
        
        up.back(conv1.diff);

        Tensor_OP().permute(sc_x, input, o_shape, x_shape, new int[] {0, 1, 3, 5, 2, 4});

        Tensor_OP().add(up.diff, input, up.diff);
