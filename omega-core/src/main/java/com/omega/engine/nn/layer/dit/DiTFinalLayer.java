...
//    public RMSLayer finalNorm;
    ...
    public void init(Tensor input) {
    	...
    	if(linearInput == null || linearInput.getShape()[0] != number) {
    		linearInput = Tensor.createGPUTensor(linearInput, number, input.getShape()[1], input.getShape()[2], input.getShape()[3], true);
    	}
    }
    ...
    public void initBack() {
        // TODO Auto-generated method stub
    	if(dShift == null || dShift.getShape()[0] != batchSize) {
    		dShift = Tensor.createGPUTensor(dShift, batchSize, 1, 1, hidden_size, true);
    	}
    	if(dScale == null || dScale.getShape()[0] != batchSize) {
    ...
    public void output(Tensor tc) {
    	...
    	Tensor_OP().mul(finalNorm.getOutput(), m_linear2.getOutput(), linearInput, batchSize, time, 1, finalNorm.getOutput().getShape()[3], 1);
    	Tensor_OP().addAxis(linearInput, m_linear1.getOutput(), linearInput, batchSize, time, 1, finalNorm.getOutput().getShape()[3], 1);
    ...
    public void diff(Tensor dtc) {
        ...
        Tensor_OP().addAxisBack(dShift, finalLinear.diff, batchSize, time, 1, finalNorm.getOutput().getShape()[3], 1);

    	Tensor_OP().mul_right_back(finalNorm.getOutput(), finalLinear.diff, dScale, batchSize, time, 1, finalNorm.getOutput().getShape()[3], 1);
    	Tensor_OP().mul_left_back(m_linear2.getOutput(), finalLinear.diff, linearInput,  batchSize, time, 1, finalNorm.getOutput().getShape()[3], 1);
