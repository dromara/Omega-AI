...
public class OpenSoraVAE extends Network {
    ...
    public void reparameterize(Tensor encode) {
        if (this.z == null || this.z.getShape()[0] != encode.getShape()[0]) {
            this.z = Tensor.createGPUTensor(this.z, encode.getShape()[0], this.latendDim * latendDepth, encode.getShape()[2], encode.getShape()[3], true);
            this.eps = Tensor.createGPUTensor(this.eps, encode.getShape()[0], this.latendDim * latendDepth, encode.getShape()[2], encode.getShape()[3], true);
            this.mu = Tensor.createGPUTensor(this.mu, encode.getShape()[0], this.latendDim * latendDepth, encode.getShape()[2], encode.getShape()[3], true);
            this.logvar = Tensor.createGPUTensor(this.logvar, encode.getShape()[0], this.latendDim * latendDepth, encode.getShape()[2], encode.getShape()[3], true);
        }
//        GPUOP.getInstance().cudaRandn(this.eps);
        this.eps.fill(1, this.tensorOP.op);
        vaeKernel.concat_channel_backward(encode, mu, logvar, encode.getShape()[0], this.latendDim, this.latendDim, latendDepth, encode.getShape()[2] * encode.getShape()[3]);
    ...
    public void reparameterize_back(Tensor delta) {
        ...
        vaeKernel.concat_channel_forward(dmu, dlogvar, encoderDelta, dmu.getShape()[0], this.latendDim, this.latendDim, latendDepth * dmu.getShape()[2], dmu.getShape()[3]);
    }

    public void initBack() {
        if (this.dlogvar == null || this.dlogvar.getShape()[0] != logvar.getShape()[0]) {
        	lpipsLossDiff = new Tensor(recLoss.getShape()[0], 1, 1, 1, MatrixUtils.val(recLoss.getShape()[0], 1.0f / recLoss.getShape()[0] * recLoss.getShape()[1] * recLoss.getShape()[2] * recLoss.getShape()[3]), true);
            this.dlogvar = Tensor.createGPUTensor(this.dlogvar, logvar.getShape()[0], this.latendDim * latendDepth, logvar.getShape()[2], logvar.getShape()[3], true);
            this.dmu = Tensor.createGPUTensor(this.dmu, mu.getShape()[0], this.latendDim * latendDepth, mu.getShape()[2], mu.getShape()[3], true);
            this.encoderDelta = Tensor.createGPUTensor(this.encoderDelta, mu.getShape()[0], this.latendDim * 2 * latendDepth, mu.getShape()[2], mu.getShape()[3], true);
        } else {
            dmu.clearGPU();
            dlogvar.clearGPU();
        }
...
//        lpipsLossDiff.showDM("lpipsLossDelta");
        ...
        rec_video.fill(1.0f/video.getShape()[0], tensorOP.op);
    ...
    public float totalLoss(Tensor output, Tensor label, Opensora_LPIPS lpips) {
        if (klLoss == null || klLoss.getShape()[0] != mu.getShape()[0]) {
        	this.lpips = lpips;
        	this.rec_video = Tensor.createTensor(this.rec_video, output.getShape()[0], decoder.oDepth, decoder.oChannel, output.getShape()[2] * output.getShape()[3], true);
        	this.video = Tensor.createTensor(this.video, output.getShape()[0], decoder.oDepth, decoder.oChannel, output.getShape()[2] * output.getShape()[3], true);
            ...
            this.recLoss = Tensor.createTensor(this.recLoss, video.getShape()[0] * video.getShape()[1], video.getShape()[2], output.getShape()[2], output.getShape()[3], true);//(b t) c h w
        }
        
        label.view(output.getShape()[0], decoder.oChannel, decoder.oDepth, output.getShape()[2] * output.getShape()[3]);
        output.view(output.getShape()[0], decoder.oChannel, decoder.oDepth, output.getShape()[2] * output.getShape()[3]);
        tensorOP.permute(label, video, new int[] {0, 2, 1, 3});
        tensorOP.permute(output, rec_video, new int[] {0, 2, 1, 3});
        label.viewOrg();
        output.viewOrg();
        video.view(video.getShape()[0] * video.getShape()[1], video.getShape()[2], output.getShape()[2], output.getShape()[3]); //(b t) c h w
        rec_video.view(rec_video.getShape()[0] * rec_video.getShape()[1], rec_video.getShape()[2], output.getShape()[2], output.getShape()[3]); //(b t) c h w
        ...
        float nll_loss = MatrixOperation.sum(recLoss.syncHost())/video.getShape()[0] + lpipLoss.syncHost()[0];
        System.out.println("nll_loss:"+nll_loss);
        
        vaeKernel.kl(mu, logvar, kl_weight, klLoss);
        float klLossf = MatrixOperation.sum(klLoss.syncHost()) / mu.getShape()[0];
