...
//        System.out.println(JsonUtils.toJson(diff.syncHost()));
    ...
    public void channel_average(Tensor input, Tensor output) {
        try {
             ...
             */
            forwardKernelParameters = Pointer.to(Pointer.to(input.getGpuData()), Pointer.to(output.getGpuData()), Pointer.to(new int[]{output.getShape()[1]}), Pointer.to(new int[]{output.getShape()[2] * output.getShape()[3]}), Pointer.to(new int[]{output.getDataLength()}));
    ...
    public void channel_average_backward(Tensor delta, Tensor diff) {
        try {
             ...
             */
            backwardKernelParameters = Pointer.to(Pointer.to(delta.getGpuData()), Pointer.to(diff.getGpuData()), Pointer.to(new int[]{delta.getShape()[1]}), Pointer.to(new int[]{delta.getShape()[2] * delta.getShape()[3]}), Pointer.to(new int[]{delta.getDataLength()}));
    ...
    public void channel_duplicate(Tensor input, Tensor output,int repeats) {
        try {
             ...
             */
            forwardKernelParameters = Pointer.to(Pointer.to(input.getGpuData()), Pointer.to(output.getGpuData()), Pointer.to(new int[]{output.getShape()[1]}), Pointer.to(new int[]{output.getShape()[2] * output.getShape()[3]}), Pointer.to(new int[]{repeats}), Pointer.to(new int[]{output.getDataLength()}));
            cuLaunchKernel(channel_duplicate_function, this.CAFFE_GET_BLOCKS(output.getDataLength()), 1, 1,      // Grid dimension
                    CAFFE_CUDA_NUM_THREADS, 1, 1,      // Block dimension
                    0, null,               // Shared memory size and stream
                    forwardKernelParameters, null // Kernel- and extra parameters
            );
    ...
    public void channel_duplicate_backward(Tensor delta, Tensor diff,int repeats) {
        try {
             ...
             */
            backwardKernelParameters = Pointer.to(Pointer.to(delta.getGpuData()), Pointer.to(diff.getGpuData()), Pointer.to(new int[]{diff.getShape()[1]}), Pointer.to(new int[]{diff.getShape()[2] * diff.getShape()[3]}), Pointer.to(new int[]{repeats}), Pointer.to(new int[]{diff.getDataLength()}));
            cuLaunchKernel(channel_duplicate_back, this.CAFFE_GET_BLOCKS(diff.getDataLength()), 1, 1,      // Grid dimension
                    CAFFE_CUDA_NUM_THREADS, 1, 1,      // Block dimension
                    0, null,               // Shared memory size and stream
                    backwardKernelParameters, null // Kernel- and extra parameters
            );
