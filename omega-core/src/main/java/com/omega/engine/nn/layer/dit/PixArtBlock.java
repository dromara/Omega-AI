...
//    	attn.getOutput().showDM("attn");
    ...
    public void modulate(Tensor x,Tensor shift,Tensor scale,Tensor output) {
    	...
    	Tensor_OP().mul(x, shift, output, batchSize, time, 1, output.getShape()[3], 1);
    ...
    public void output(Tensor tc,Tensor text,Tensor cos,Tensor sin, Tensor[] ts) {
    	...
    	Tensor_OP().mul(attn.getOutput(), gate_msa, crossAttnInput, batchSize, time, 1, crossAttnInput.getShape()[3], 1);
    	...
    	Tensor_OP().mul(mlp.getOutput(), gate_mlp, output, batchSize, time, 1, output.getShape()[3], 1);
