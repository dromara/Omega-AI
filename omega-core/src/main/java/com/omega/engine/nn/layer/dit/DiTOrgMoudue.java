...
//    	System.err.println("emb:"+JsonUtils.toJson(emb));
    ...
    public void initBack() {
        // TODO Auto-generated method stub
    	if(dtc == null || dtc.getShape()[0] != timeEmbd.getOutput().getShape()[0]) {
    		dtc = Tensor.createGPUTensor(dtc, timeEmbd.getOutput().shape(), true);
    		dtext = Tensor.createGPUTensor(dtext, timeEmbd.getOutput().getShape()[0] * maxContextLen, 1, 1, hiddenSize, true);
    	}else {
    		dtext.clearGPU();
    		dtc.clearGPU();
    	}
    ...
    public void output(Tensor tc,Tensor text) {
    	...
    	Tensor_OP().addAxis(patchEmbd.getOutput(), posEmbd, patchEmbd.getOutput(), patchEmbd.getOutput().getShape()[0], patchEmbd.getOutput().getShape()[1], 1, patchEmbd.getOutput().getWidth(), 1);

    	timeEmbd.forward(tc);
    	
    	labelEmbd.forward(text);

    	Tensor x = patchEmbd.getOutput().view(patchEmbd.getOutput().getShape()[0] * patchEmbd.getOutput().getShape()[1], 1, 1, patchEmbd.getOutput().getShape()[3]);
