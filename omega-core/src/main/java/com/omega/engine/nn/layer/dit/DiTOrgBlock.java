...
//    public RMSLayer norm1;
    ...
    public void init(Tensor input) {
        ...
        if(crossAttnInput == null || crossAttnInput.getShape()[0] != number) {
        	...
        	crossAttnOut = Tensor.createGPUTensor(crossAttnOut, input.getShape()[0], input.getShape()[1], input.getShape()[2], input.getShape()[3], true);
        }
    ...
    public void modulate(Tensor x,Tensor shift,Tensor scale,Tensor output) {
    	...
    	Tensor_OP().mul(x, scale, output, batchSize, time, 1, output.getShape()[3], 1);
    	Tensor_OP().addAxis(output, shift, output, batchSize, time, 1, output.getShape()[3], 1);
    }
    ...
    public void output(Tensor tc,Tensor text) {
    	...
    	Tensor_OP().mul(attn.getOutput(), modulation_gate_msa.getOutput(), crossAttnInput, batchSize, time, 1, crossAttnInput.getShape()[3], 1);
    	...
    	Tensor_OP().mul(mlp.getOutput(), modulation_gate_mlp.getOutput(), output, batchSize, time, 1, output.getShape()[3], 1);
    ...
    public void modulate_back(Tensor dShift,Tensor dScale,Tensor dx,Tensor x,Tensor scale,Tensor delta) {
    	Tensor_OP().addAxisBack(dShift, delta, batchSize, time, 1, delta.getShape()[3], 1);
    	Tensor_OP().mul_right_back(x, delta, dScale, batchSize, time, 1, delta.getShape()[3], 1);
    	Tensor_OP().mul_left_back(scale, delta, dx,  batchSize, time, 1, delta.getShape()[3], 1);
    }
    
    public void diff(Tensor dtc,Tensor dText) {
    	
    	Tensor_OP().mul_left_back(modulation_gate_mlp.getOutput(), delta, output,  batchSize, time, 1, output.getShape()[3], 1);
    	mlp.back(output);
    	Tensor_OP().mul_right_back(mlp.getOutput(), delta, modulation_gate_mlp.getOutput(), batchSize, time, 1, output.getShape()[3], 1);
    	...
    	Tensor_OP().mul_left_back(modulation_gate_msa.getOutput(), norm2.diff, output,  batchSize, time, 1, output.getShape()[3], 1);
    	attn.back(output);
    	Tensor_OP().mul_right_back(attn.getOutput(), norm2.diff, modulation_gate_msa.getOutput(), batchSize, time, 1, output.getShape()[3], 1);
