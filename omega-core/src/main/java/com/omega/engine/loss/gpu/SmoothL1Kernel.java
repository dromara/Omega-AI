...
public class SmoothL1Kernel extends BaseKernel {
    ...
    public void forward(Tensor input, Tensor currentLabel, Tensor output, float beta) {
         ...
         */
        loss_kernelParameters = Pointer.to(Pointer.to(new int[]{input.getDataLength()}), Pointer.to(input.getGpuData()), Pointer.to(currentLabel.getGpuData()), Pointer.to(output.getGpuData()), Pointer.to(new float[]{beta}));
        cuLaunchKernel(loss_function,  this.CAFFE_GET_BLOCKS(input.getDataLength()), 1, 1,      // Grid dimension
    ...
    public void backward(Tensor input, Tensor currentLabel, Tensor diff, float beta) {
         ...
         */
        backKernelParameters = Pointer.to(Pointer.to(new int[]{input.getDataLength()}), Pointer.to(input.getGpuData()), Pointer.to(currentLabel.getGpuData()), Pointer.to(diff.getGpuData()), Pointer.to(new float[]{beta}));
        cuLaunchKernel(loss_backward_function, this.CAFFE_GET_BLOCKS(input.getDataLength()), 1, 1,      // Grid dimension
                CAFFE_CUDA_NUM_THREADS, 1, 1,      // Block dimension
                0, null,               // Shared memory size and stream
