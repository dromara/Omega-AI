...
public class MSELossKernel extends BaseKernel {
    ...
    public void backward(Tensor input, Tensor currentLabel, Tensor diff) {
         ...
         */
        backKernelParameters = Pointer.to(Pointer.to(input.getGpuData()), Pointer.to(currentLabel.getGpuData()), Pointer.to(diff.getGpuData()), Pointer.to(new int[]{input.getShape()[1] * input.getShape()[2] * input.getShape()[3]}), Pointer.to(new int[]{input.getShape()[0]}));
