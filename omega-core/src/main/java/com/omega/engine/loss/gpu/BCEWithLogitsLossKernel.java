...
public class BCEWithLogitsLossKernel extends BaseKernel {
    ...
    public BCEWithLogitsLossKernel(CUDAManager cudaManager) {
        super(cudaManager);
        init();
    }
    ...
    public void initFunction() {
        try {
            if (loss_function == null) {
                loss_function = getCudaManager().getLocalFunctionByModule("BCEWithLogitsLossKernel.cu", "loss");
            }
            if (loss_backward_function == null) {
                loss_backward_function = getCudaManager().getLocalFunctionByModule("BCEWithLogitsLossKernel.cu", "loss_back");
            }
        } catch (Exception e) {
