...
public class TrainDataLoader {
    ...
    private static void loadDataByIdx(Integer[] idx, DataSet org, DataSet dest) {
        ...
        for (int i = 0; i < idx.length; i++) {
            int index = idx[i];
            System.arraycopy(org.input.getData(), index * channel * height * width, dest.input.getData(), i * channel * height * width, channel * height * width);
            System.arraycopy(org.label.getData(), index * labelSize, dest.label.getData(), i * labelSize, labelSize);
