...
public class AnchorBoxUtils {
    public static Tensor sort(Tensor centers) {
        Tensor result = new Tensor(centers.getShape()[0], 1, 1, centers.getShape()[3]);
        for (int i = 0; i < centers.getShape()[0]; i++) {
            float w = centers.getData()[i * 2 + 0];
            float h = centers.getData()[i * 2 + 1];
            float product = w * h;
            int j;
            for (j = i; j > 0 && product < (centers.getData()[(j - 1) * 2 + 0] * centers.getData()[(j - 1) * 2 + 1]); j--) {
                centers.getData()[j * 2 + 0] = centers.getData()[(j - 1) * 2 + 0];
                centers.getData()[j * 2 + 1] = centers.getData()[(j - 1) * 2 + 1];
            }
            centers.getData()[j * 2 + 0] = w;
            centers.getData()[j * 2 + 1] = h;
        }
        return result;
    }
    ...
    public static Tensor calcAnchors(Tensor bboxs, int num_clusters) {
        int num_labels = bboxs.getShape()[0];
        Tensor wh = new Tensor(num_labels, 1, 1, 2);
        int[] assig = new int[wh.getShape()[0]];
        for (int n = 0; n < num_labels; n++) {
            wh.getData()[n * 2 + 0] = bboxs.getData()[n * 4 + 2];
            wh.getData()[n * 2 + 1] = bboxs.getData()[n * 4 + 3];
        }
    ...
    public static void kmeans_max(Tensor wh, int[] assig, Tensor centers) {
        float[] old_centers = new float[centers.getDataLength()];
        int[] counts = new int[centers.getShape()[0]];
        for (int i = 0; i < centers.getShape()[0]; i++) {
            for (int j = 0; j < centers.getOnceSize(); j++) {
                old_centers[i * centers.getOnceSize() + j] = centers.getData()[i * wh.getOnceSize() + j];
                centers.getData()[i * centers.getOnceSize() + j] = 0;
            }
        }
        for (int i = 0; i < wh.getShape()[0]; i++) {
            counts[assig[i]]++;
            for (int j = 0; j < wh.getOnceSize(); j++) {
                centers.getData()[assig[i] * centers.getOnceSize() + j] += wh.getData()[i * wh.getOnceSize() + j];
            }
        }
        for (int i = 0; i < centers.getShape()[0]; i++) {
            if (counts[i] > 0) {
                for (int j = 0; j < centers.getOnceSize(); j++) {
                    centers.getData()[i * centers.getOnceSize() + j] /= counts[i];
                }
            }
        }
        for (int i = 0; i < centers.getShape()[0]; i++) {
            for (int j = 0; j < centers.getOnceSize(); j++) {
                if (centers.getData()[i * centers.getOnceSize() + j] == 0) {
                    centers.getData()[i * centers.getOnceSize() + j] = old_centers[i * centers.getOnceSize() + j];
                }
            }
        }
    }
    ...
    public static int kmeans_exp(Tensor wh, int[] assig, Tensor centers) {
        ...
        for (int i = 0; i < wh.getShape()[0]; i++) {
            ...
            datum[0] = wh.getData()[i * 2 + 0];
            datum[1] = wh.getData()[i * 2 + 1];
    ...
    public static Tensor randomCenters(Tensor wh, int k) {
        ...
        for (int n = 0; n < k; n++) {
            System.arraycopy(wh.getByNumber(list.get(n)), 0, centers.getData(), n * wh.getOnceSize(), wh.getOnceSize());
        }
        return centers;
    }
    ...
    public static Tensor translateBoxes(Tensor boxs) {
        Tensor wh = new Tensor(boxs.getShape()[0], 1, 1, boxs.getShape()[3] / 2);
        for (int n = 0; n < boxs.getShape()[0]; n++) {
            wh.getData()[n * wh.getShape()[3] + 0] = Math.abs(boxs.getData()[n * boxs.getShape()[3] + 2] - boxs.getData()[n * boxs.getShape()[3] + 0]);
            wh.getData()[n * wh.getShape()[3] + 1] = Math.abs(boxs.getData()[n * boxs.getShape()[3] + 3] - boxs.getData()[n * boxs.getShape()[3] + 1]);
        }
        return wh;
    }

    public static float[] iou(float[] box, Tensor clusters) {
        float[] iou = new float[clusters.getShape()[0]];
        for (int k = 0; k < clusters.getShape()[0]; k++) {
            float x = clusters.getData()[k * 2 + 0] < box[0] ? clusters.getData()[k * 2 + 0] : box[0];
            float y = clusters.getData()[k * 2 + 1] < box[1] ? clusters.getData()[k * 2 + 1] : box[1];
            float intersection = x * y;
            float box_area = box[0] * box[1];
            float cluster_area = clusters.getData()[k * 2 + 0] * clusters.getData()[k * 2 + 1];
