...
public class YoloDecode {
    ...
    public static float[][][] getDetection(Tensor x, int w, int h) {
        ...
        float[][][] dets = new float[x.getShape()[0]][location][1 + class_number + 4];
        for (int b = 0; b < x.getShape()[0]; b++) {
            ...
            for (int l = 0; l < location; ++l) {
                ...
                for (int n = 0; n < bbox_num; ++n) {
                    ...
                    float scale = x.getData()[confidence_index];
                    int box_index = input_index + location * (class_number + bbox_num) + (l * bbox_num + n) * 4;
                    float[] bbox = new float[class_number + 1 + 4];
                    bbox[class_number + 1] = (x.getData()[box_index + 0] + col) / grid_size * w;
                    bbox[class_number + 2] = (x.getData()[box_index + 1] + row) / grid_size * h;
                    bbox[class_number + 3] = (float) (Math.pow(x.getData()[box_index + 2], 2.0f) * w);
                    bbox[class_number + 4] = (float) (Math.pow(x.getData()[box_index + 3], 2.0f) * h);
                    for (int j = 0; j < class_number; ++j) {
                        float prob = scale * x.getData()[class_index + j];
    ...
    public static float[][][] getDetectionLabel(Tensor x, int w, int h) {
        ...
        for (int b = 0; b < x.getShape()[0]; b++) {
            ...
            for (int l = 0; l < location; ++l) {
                ...
                bbox[class_number + 1] = (x.getData()[box_index + 0] + col) / grid_size * w;
                bbox[class_number + 2] = (x.getData()[box_index + 1] + row) / grid_size * h;
                bbox[class_number + 3] = (float) (x.getData()[box_index + 2] * w);
                bbox[class_number + 4] = (float) (x.getData()[box_index + 3] * h);
                bbox[0] = scale;
                for (int j = 0; j < class_number; ++j) {
                    float prob = scale * x.getData()[class_index + j];
    ...
    public static float[][][] getDetectionLabelYolov3(Tensor x, int w, int h) {
        ...
        float[][][] dets = new float[x.getShape()[0]][90][1 + class_number + 4];
        for (int b = 0; b < x.getShape()[0]; b++) {
            ...
            for (int l = 0; l < 90; ++l) {
                ...
                float[] bbox = new float[class_number + 1 + 4];
                if (x.getData()[box_index + 2] != 0 && x.getData()[box_index + 3] != 0) {
                    bbox[class_number + 1] = x.getData()[box_index + 0] * w;
                    bbox[class_number + 2] = x.getData()[box_index + 1] * h;
                    bbox[class_number + 3] = x.getData()[box_index + 2] * w;
                    bbox[class_number + 4] = x.getData()[box_index + 3] * h;
                    bbox[0] = 1;
                    System.out.println(b + ":" + JsonUtils.toJson(bbox));
                }
                score_bbox[l] = bbox;
