...
public class LabelUtils {
    public static void loadBoxCSV(String labelPath, Tensor box) {
        try (FileInputStream fin = new FileInputStream(labelPath); InputStreamReader reader = new InputStreamReader(fin); BufferedReader buffReader = new BufferedReader(reader);) {
            String strTmp = "";
            int idx = 0;
            int onceSize = box.getShape()[1] * box.getShape()[2] * box.getShape()[3];
            while ((strTmp = buffReader.readLine()) != null) {
                if (idx > 0) {
                    String[] list = strTmp.split(" ");
                    for (int i = 2; i < list.length; i++) {
                        box.getData()[(idx - 1) * onceSize + i - 2] = Float.parseFloat(list[i]);
                    }
                }
                idx++;
            }
    ...
    public static void loadLabelCSV(String labelPath, Tensor label, String[] idxs) {
        try (FileInputStream fin = new FileInputStream(labelPath); InputStreamReader reader = new InputStreamReader(fin); BufferedReader buffReader = new BufferedReader(reader);) {
            String strTmp = "";
            int idx = 0;
            int onceSize = label.getShape()[1] * label.getShape()[2] * label.getShape()[3];
            while ((strTmp = buffReader.readLine()) != null) {
                ...
                if (idx > 0) {
                    ...
                    for (int i = 1; i < list.length; i++) {
                        label.getData()[(idx - 1) * onceSize + i - 1] = Float.parseFloat(list[i]);
                    }
                }
                idx++;
            }
    ...
    public static void loadLabel(String labelPath, Tensor label) {
        try (FileInputStream fin = new FileInputStream(labelPath); InputStreamReader reader = new InputStreamReader(fin); BufferedReader buffReader = new BufferedReader(reader);) {
            ...
            while ((strTmp = buffReader.readLine()) != null) {
                String[] list = strTmp.split(" ");
                for (int i = 1; i < list.length; i++) {
                    label.getData()[idx * onceSize + i - 1] = Float.parseFloat(list[i]);
                }
                idx++;
            }
        } catch (Exception e) {
