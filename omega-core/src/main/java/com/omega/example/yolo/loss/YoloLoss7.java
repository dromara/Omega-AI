...
public class YoloLoss7 extends LossFunction {
    ...
    public static float[] getYoloBox(Tensor x, float[] anchors, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride) {
        ...
        box[2] = (float) x.getData()[index + 2 * stride] * x.getData()[index + 2 * stride] * 4 * anchors[2 * n] / w;
        box[3] = (float) x.getData()[index + 3 * stride] * x.getData()[index + 3 * stride] * 4 * anchors[2 * n + 1] / h;
        return box;
    }
        ...
        for (int b = 0; b < x.getShape()[0]; b++) {
            ...
            for (int h = 0; h < x.getShape()[2]; h++) {
                for (int w = 0; w < x.getShape()[3]; w++) {
                    for (int n = 0; n < this.bbox_num; n++) {
                        ...
                        int class_index = entryIndex(b, x.getShape()[3], x.getShape()[2], n_index, 4 + 1);
                        ...
                        for (int t = 0; t < maxBox; t++) {
                            ...
                            int class_id = (int) label.getData()[t * (4 + 1) + b * truths + 4];
                            if (class_id >= class_number || class_id < 0) {
                                System.err.println("error class.");
                            }
                            float objectness = x.getData()[obj_index];
                            if (Float.isNaN(objectness) || Float.isInfinite(objectness)) {
                                x.getData()[obj_index] = 0;
                            }
                            int class_id_match = compareYoloClass(x, this.class_number, class_index, x.getShape()[3] * x.getShape()[2], objectness, class_id, 0.25f);
                        ...
                        this.diff.getData()[obj_index] = this.obj_normalizer * x.getData()[obj_index];
                        if (best_match_iou > ignoreThresh) {
                            ...
                            if (objectness_smooth == 1) {
                                ...
                                float delta_obj = obj_normalizer * (x.getData()[obj_index] - best_match_iou);
                                if (delta_obj > this.diff.getData()[obj_index]) {
                                    this.diff.getData()[obj_index] = delta_obj;
                                }
                            } else {
            ...
            for (int t = 0; t < maxBox; t++) {
                ...
                if (mask_n >= 0) {
                    ...
                    float iou = deltaYoloBox(truth, x, anchors, bestIndex, box_index, i, j, x.getShape()[3], x.getShape()[2], stride);
                    int obj_index = entryIndex(b, x.getShape()[3], x.getShape()[2], mask_n_index, 4);
                    if (x.getData()[obj_index] >= 0.7f) {
                        testCount++;
                    }
                    avg_obj += x.getData()[obj_index];
                    if (objectness_smooth == 1) {
                        if (this.diff.getData()[obj_index] == 0) {
                            this.diff.getData()[obj_index] = obj_normalizer * (x.getData()[obj_index] - 1.0f);
                        }
                    } else {
                        this.diff.getData()[obj_index] = obj_normalizer * (x.getData()[obj_index] - 1.0f);
                    }
                ...
                for (int n = 0; n < total; ++n) {
                    ...
                    if (mask_n_t >= 0 && n != bestIndex && this.iou_thresh < 1.0f) {
                        ...
                        if (iou > iou_thresh) {
                            int mask_n_index = mask_n_t * x.getShape()[3] * x.getShape()[2] + j * x.getShape()[3] + i;
                            int class_id = (int) label.getData()[t * (4 + 1) + b * truths + 4];
                            int box_index = entryIndex(b, x.getShape()[3], x.getShape()[2], mask_n_index, 0);
                            float ciou = deltaYoloBox(truth, x, anchors, n, box_index, i, j, x.getShape()[3], x.getShape()[2], stride);
                            int obj_index = entryIndex(b, x.getShape()[3], x.getShape()[2], mask_n_index, 4);
                            if (x.getData()[obj_index] >= 0.7f) {
                                testCount++;
                            }
    ...
    private float deltaYoloBox(float[] truth, Tensor x, float[] anchors, int n, int index, int i, int j, int lw, int lh, int stride) {
        ...
        this.diff.getData()[index + 0 * stride] -= dx[0];
        this.diff.getData()[index + 1 * stride] -= dx[1];
        this.diff.getData()[index + 2 * stride] -= dx[2];
        this.diff.getData()[index + 3 * stride] -= dx[3];
        return ciou;
    }
    ...
    private void averagesYoloDeltas(int class_index, int box_index, int stride, int classes, Tensor delta) {
        ...
        for (int c = 0; c < classes; ++c) {
            if (delta.getData()[class_index + stride * c] > 0) {
                classes_in_one_box++;
            }
        }
        //	    System.out.println(classes_in_one_box);
        if (classes_in_one_box > 0) {
            delta.getData()[box_index + 0 * stride] /= classes_in_one_box;
            delta.getData()[box_index + 1 * stride] /= classes_in_one_box;
            delta.getData()[box_index + 2 * stride] /= classes_in_one_box;
            delta.getData()[box_index + 3 * stride] /= classes_in_one_box;
        }
    }

    private float deltaYoloClass(Tensor output, int index, int class_id, int classes, int stride, float avg_cat) {
        if (this.diff.getData()[index + stride * class_id] == 1.0f) {
            float y_true = 1;
            //			float result_delta = y_true - output.data[index + stride*class_id];
            float result_delta = output.getData()[index + stride * class_id] - y_true;
            if (!Float.isNaN(result_delta) && !Float.isInfinite(result_delta)) {
                this.diff.getData()[index + stride * class_id] = result_delta;
            }
            avg_cat += output.getData()[index + stride * class_id];
            return avg_cat;
        }
        if (this.focal_loss == 1) {
            ...
            float pt = output.getData()[ti] + 0.000000000000001f;
            ...
            for (int n = 0; n < classes; ++n) {
                ...
                this.diff.getData()[index + stride * n] = output.getData()[index + stride * n] - y_true;
                this.diff.getData()[index + stride * n] *= alpha * grad;
                if (n == class_id) {
                    avg_cat += output.getData()[index + stride * n];
                }
            }
        } else {
            for (int n = 0; n < classes; n++) {
                ...
                float result_delta = output.getData()[index + stride * n] - y_true;
                //				if(y_true == 1){
                //					System.out.println(output.data[index + stride * n] + ":" + result_delta);
                //				}
                if (!Float.isNaN(result_delta) && !Float.isInfinite(result_delta)) {
                    this.diff.getData()[index + stride * n] = result_delta;
                }
    ...
    private int compareYoloClass(Tensor output, int classes, int class_index, int stride, float obj, int class_id, float conf_thresh) {
        for (int j = 0; j < classes; ++j) {
            float prob = output.getData()[class_index + stride * j];
