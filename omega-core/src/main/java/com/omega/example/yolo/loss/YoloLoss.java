        ...
        for (int b = 0; b < x.getShape()[0]; b++) {
            ...
            for (int l = 0; l < location; l++) {
                for (int n = 0; n < bbox_num; n++) {
                    ...
                    this.diff.getData()[confidence_index] = noobject_scale * (x.getData()[confidence_index]);
                    cost += noobject_scale * Math.pow(x.getData()[confidence_index], 2.0f);
                    avg_anyobj += x.getData()[confidence_index];
                }
                int truth_index = (class_number + 4 + 1) * l + b * truth_num_each;
                if (label.getData()[truth_index] != 1.0f) {
                    continue;
                }
                ...
                for (int j = 0; j < class_number; ++j) {
                    cost += class_scale * Math.pow(x.getData()[class_index + j] - label.getData()[truth_index + 1 + j], 2);
                    this.diff.getData()[class_index + j] = class_scale * (x.getData()[class_index + j] - label.getData()[truth_index + 1 + j]);
                    if (label.getData()[truth_index + 1 + j] == 1.0f) {
                        avg_cat += x.getData()[class_index + j];
                    }
                    avg_allcat += x.getData()[class_index + j];
                }
                //获取gt bbox
                float[] truthCoords = new float[4];
                truthCoords[0] = label.getData()[truth_index + 1 + class_number + 0] / grid_number;
                truthCoords[1] = label.getData()[truth_index + 1 + class_number + 1] / grid_number;
                truthCoords[2] = label.getData()[truth_index + 1 + class_number + 2];
                truthCoords[3] = label.getData()[truth_index + 1 + class_number + 3];
                ...
                for (int n = 0; n < bbox_num; n++) {
                    ...
                    bbox[0] = x.getData()[inputCoordsIndex + 0] / grid_number;  //x
                    bbox[1] = x.getData()[inputCoordsIndex + 1] / grid_number;  //y
                    bbox[2] = x.getData()[inputCoordsIndex + 2] * x.getData()[inputCoordsIndex + 2];  //w = w*w
                    bbox[3] = x.getData()[inputCoordsIndex + 3] * x.getData()[inputCoordsIndex + 3];  //h = h*h
                ...
                avg_iou += best_iou;
                cost += coord_scale * Math.pow(x.getData()[best_coords + 0] - label.getData()[t_bbox_index + 0], 2);
                cost += coord_scale * Math.pow(x.getData()[best_coords + 1] - label.getData()[t_bbox_index + 1], 2);
                cost += coord_scale * Math.pow(x.getData()[best_coords + 2] - Math.sqrt(label.getData()[t_bbox_index + 2]), 2);
                cost += coord_scale * Math.pow(x.getData()[best_coords + 3] - Math.sqrt(label.getData()[t_bbox_index + 3]), 2);
                //	            cost += Math.pow(1.0f - best_iou, 2.0f);
                this.diff.getData()[best_coords + 0] = coord_scale * (x.getData()[best_coords + 0] - label.getData()[t_bbox_index + 0]);
                this.diff.getData()[best_coords + 1] = coord_scale * (x.getData()[best_coords + 1] - label.getData()[t_bbox_index + 1]);
                this.diff.getData()[best_coords + 2] = (float) (coord_scale * (x.getData()[best_coords + 2] - Math.sqrt(label.getData()[t_bbox_index + 2])));
                this.diff.getData()[best_coords + 3] = (float) (coord_scale * (x.getData()[best_coords + 3] - Math.sqrt(label.getData()[t_bbox_index + 3])));
                //计算loss函数第3项
                //先减去计算第4项lost时多加上的有物体的网格的置信度
                int confidence_index = input_index + location * class_number + l * bbox_num + n_best;
                cost -= noobject_scale * Math.pow(0.0f - x.getData()[confidence_index], 2);
                cost += object_scale * Math.pow(x.getData()[confidence_index] - 1.0f, 2);
                this.diff.getData()[confidence_index] = object_scale * (x.getData()[confidence_index] - 1.0f);
                //	            this.diff.data[confidence_index] = object_scale*(x.data[confidence_index] - best_iou);
                avg_obj += x.getData()[confidence_index];
                count++;
            }
        }
        System.out.println("Detection Avg IOU:" + avg_iou / count + ",Pos Cat:" + avg_cat / count + ",All Cat:" + avg_allcat / (class_number * count) + ",Pos Obj:" + avg_obj / count + ",Any Obj:" + avg_anyobj / (location * x.getShape()[0] * bbox_num) + ",count:" + count);
        this.loss.getData()[0] = cost;
