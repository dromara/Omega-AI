...
public class YoloLoss2 extends LossFunction {
    ...
    public Tensor loss(Tensor x, Tensor label) {
        ...
        for (int b = 0; b < x.getShape()[0]; b++) {
            for (int h = 0; h < x.getShape()[2]; h++) {
                for (int w = 0; w < x.getShape()[3]; w++) {
                    for (int n = 0; n < this.bbox_num; n++) {
                        ...
                        float[] pred = getYoloBox(x, anchors, n, box_index, w, h, x.getShape()[3], x.getShape()[2], orgW, orgH, stride);
                        float bestIOU = 0;
                        ...
                        this.diff.getData()[obj_index] = this.noobject_scale * x.getData()[obj_index];
            ...
            for (int t = 0; t < maxBox; t++) {
                ...
                int box_index = entryIndex(b, x.getShape()[3], x.getShape()[2], bestIndex * x.getShape()[3] * x.getShape()[2] + j * x.getShape()[3] + i, 0);
                float iou = deltaYoloBox(truth, x, anchors, bestIndex, box_index, i, j, x.getShape()[3], x.getShape()[2], coord_scale * (2.0f - truth[2] * truth[3]), stride);
                if (iou > .5)
                    recall += 1;
                avg_iou += iou;
                int obj_index = entryIndex(b, x.getShape()[3], x.getShape()[2], bestIndex * x.getShape()[3] * x.getShape()[2] + j * x.getShape()[3] + i, 4);
                avg_obj += x.getData()[obj_index];
                this.diff.getData()[obj_index] = object_scale * (x.getData()[obj_index] - 1.0f);
                //                this.diff.data[obj_index] = object_scale * (x.data[obj_index] - iou);
                int clazz = (int) label.getData()[t * (4 + 1) + b * truths + 4];
                int class_index = entryIndex(b, x.getShape()[3], x.getShape()[2], bestIndex * x.getShape()[3] * x.getShape()[2] + j * x.getShape()[3] + i, 4 + 1);
                avg_cat = deltaYoloClass(x, class_index, clazz, class_number, stride, avg_cat);
                ++count;
                ++class_count;
            }
        }
        System.out.println("loss:" + Math.pow(mag_array(this.diff.getData()), 2.0) / x.getShape()[0] + ", Avg IOU: " + avg_iou / count + ", Class: " + avg_cat / class_count + ", Obj: " + avg_obj / count + "," + " No Obj: " + avg_anyobj / (x.getShape()[3] * x.getShape()[2] * bbox_num * x.getShape()[0]) + ", Avg Recall: " + recall / count + ", count: " + count);
        return loss;
    }
    ...
    private float deltaYoloClass(Tensor x, int index, int clazz, int classes, int stride, float avg_cat) {
        if (this.diff.getData()[index] == 1.0f) {
            this.diff.getData()[index + stride * clazz] = 1.0f - x.getData()[index + stride * clazz];
    ...
    private void deltaYoloMask(float[] truth, Tensor x, int n, int index, int stride, int scale) {
        for (int i = 0; i < n; i++) {
            this.diff.getData()[index + i * stride] = scale * (truth[i] - x.getData()[index + i * stride]);
        }
    }
