...
public class YoloV7Test {
    ...
    public void getAnchors() {
        try {
            ...
            for (int i = 0; i < k; i++) {
                float w = anchors.getData()[i * 2 + 0];
                float h = anchors.getData()[i * 2 + 1];
                System.out.println("w:" + w + "*h:" + h + "=" + w * h);
            }
            for (int i = 0; i < k; i++) {
                int w = (int) anchors.getData()[i * 2 + 0];
                int h = (int) anchors.getData()[i * 2 + 1];
                System.out.print(w + "," + h + ",");
            }
            System.out.println("");
            System.out.println(JsonUtils.toJson(anchors.getData()));
        } catch (Exception e) {
            // TODO: handle exception
            e.printStackTrace();
        }
