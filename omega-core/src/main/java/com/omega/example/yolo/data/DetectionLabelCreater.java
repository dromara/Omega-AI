...
public class DetectionLabelCreater implements Serializable {
    ...
    public void clearData() {
        this.label.setData(new float[this.label.getDataLength()]);
    }
    ...
    private void loadLabelToYolov1(String key, int index) {
        ...
        for (int n = 0; n < list.size(); n++) {
             ...
             */
            label.getData()[index * oneSize + gridy * stride * once + gridx * once + 0] = 1.0f;
            /**
             * class

             */
            label.getData()[index * oneSize + gridy * stride * once + gridx * once + clazz] = 1.0f;
            /**
             * x1,y1,w1,h1

             */
            label.getData()[index * oneSize + gridy * stride * once + gridx * once + class_num + 1] = px;
            label.getData()[index * oneSize + gridy * stride * once + gridx * once + class_num + 2] = py;
            label.getData()[index * oneSize + gridy * stride * once + gridx * once + class_num + 3] = w;
            label.getData()[index * oneSize + gridy * stride * once + gridx * once + class_num + 4] = h;
        }
    }
