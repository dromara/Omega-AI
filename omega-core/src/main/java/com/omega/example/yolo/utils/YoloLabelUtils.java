...
public class YoloLabelUtils {
    ...
    public static void formatToYoloV3(YoloDataLoader dataLoader, int im_w, int im_h, boolean format) {
        if (format) {
            dataLoader.getLabelSet().setData(labelToYoloV3(dataLoader.getLabelSet(), dataLoader.getLabelSet().getShape()[0], im_w, im_h));
        } else {
            dataLoader.getLabelSet().setData(labelToYoloV3NotFormat(dataLoader.getLabelSet(), dataLoader.getLabelSet().getShape()[0], im_w, im_h));
        }
    }

    public static DataSet formatToYoloV3(DataSet dataSet, int im_w, int im_h) {
        dataSet.label.setData(labelToYoloV3(dataSet.label, dataSet.label.getShape()[0], im_w, im_h));
        return dataSet;
    }

    public static DataSet formatToYolo(DataSet dataSet, int im_w, int im_h) {
        dataSet.label.setData(labelToYolo(dataSet.label.getData(), dataSet.label.getShape()[0], 7, im_w, im_h));
        dataSet.label.getShape()[3] = 7 * 7 * 6;
        dataSet.labelSize = 7 * 7 * 6;
        return dataSet;
    }

    public static Tensor formatToYolo(Tensor label, int im_w, int im_h) {
        label.setData(labelToYolo(label.getData(), label.getShape()[0], 7, im_w, im_h));
        label.getShape()[3] = 7 * 7 * 6;
        return label;
    }

    public static Tensor formatToYolo(Tensor label, int im_w, int im_h, int classNum) {
        label.setData(labelToYolo(label.getData(), label.getShape()[0], classNum, 7, im_w, im_h));
        label.getShape()[3] = 7 * 7 * (5 + classNum);
        return label;
    }

    public static DataSet formatToYoloSetClass(DataSet dataSet, int classNum, int im_w, int im_h, boolean format) {
        if (format) {
            dataSet.label.setData(labelToYolo(dataSet.label.getData(), dataSet.label.getShape()[0], classNum, 7, im_w, im_h));
        } else {
            dataSet.label.setData(labelToYoloNotFormat(dataSet.label.getData(), dataSet.label.getShape()[0], classNum, 7, im_w, im_h));
        }
        dataSet.label.getShape()[3] = 7 * 7 * (5 + classNum);
        return dataSet;
    }

    public static void formatToYoloSetClass(YoloDataLoader dataLoader, int classNum, int im_w, int im_h, boolean format) {
        if (format) {
            dataLoader.getLabelSet().setData(labelToYolo(dataLoader.getLabelSet(), dataLoader.getLabelSet().getShape()[0], classNum, 7, im_w, im_h));
        } else {
            dataLoader.getLabelSet().setData(labelToYoloNotFormat(dataLoader.getLabelSet(), dataLoader.getLabelSet().getShape()[0], classNum, 7, im_w, im_h));
        }
        dataLoader.getLabelSet().getShape()[3] = 7 * 7 * (5 + classNum);
        dataLoader.labelSize = 7 * 7 * (5 + classNum);
    }

    public static Tensor formatToYoloV3(Tensor label, int im_w, int im_h) {
        label.setData(labelToYoloV3(label, label.getShape()[0], im_w, im_h));
    ...
    public static float[] labelToYolo(Tensor label, int number, int stride, int im_w, int im_h) {
        ...
        for (int i = 0; i < number; i++) {
            float x1 = label.getData()[i * 5 + 1];
            float y1 = label.getData()[i * 5 + 2];
            float x2 = label.getData()[i * 5 + 3];
            float y2 = label.getData()[i * 5 + 4];
    ...
    public static float[] labelToYolo(Tensor bbox, int number, int classNum, int stride, int im_w, int im_h) {
        ...
        for (int b = 0; b < number; b++) {
            for (int n = 0; n < boxNum; n++) {
                int clazz = (int) bbox.getData()[b * width + n * 5 + 0] + 1;
                float x1 = bbox.getData()[b * width + n * 5 + 1];
                float y1 = bbox.getData()[b * width + n * 5 + 2];
                float x2 = bbox.getData()[b * width + n * 5 + 3];
                float y2 = bbox.getData()[b * width + n * 5 + 4];
                float cx = (x1 + x2) / (2 * im_w);
                float cy = (y1 + y2) / (2 * im_h);
                float w = (x2 - x1) / im_w;
                float h = (y2 - y1) / im_h;
    ...
    public static float[] labelToYoloNotFormat(Tensor bbox, int number, int classNum, int stride, int im_w, int im_h) {
        ...
        for (int b = 0; b < number; b++) {
            for (int n = 0; n < boxNum; n++) {
                int clazz = (int) bbox.getData()[b * width + n * 5 + 0] + 1;
                float cx = bbox.getData()[b * width + n * 5 + 1] / im_w;
                float cy = bbox.getData()[b * width + n * 5 + 2] / im_h;
                float w = bbox.getData()[b * width + n * 5 + 3] / im_w;
                float h = bbox.getData()[b * width + n * 5 + 4] / im_h;
                if (w == 0 || h == 0) {
                    break;
                }
                int gridx = (int) (cx * stride);
    ...
    public static float[] labelToYoloV3(Tensor label, int number, int im_w, int im_h) {
        int channel = label.getShape()[3] / 5;
        float[] target = new float[number * 5 * channel];
        for (int i = 0; i < number; i++) {
            for (int c = 0; c < channel; c++) {
                float clazz = label.getData()[i * channel * 5 + c * 5 + 0];
                float x1 = label.getData()[i * channel * 5 + c * 5 + 1];
                float y1 = label.getData()[i * channel * 5 + c * 5 + 2];
                float x2 = label.getData()[i * channel * 5 + c * 5 + 3];
                float y2 = label.getData()[i * channel * 5 + c * 5 + 4];
                float cx = (x1 + x2) / (2 * im_w);
                float cy = (y1 + y2) / (2 * im_h);
                float w = (x2 - x1) / im_w;
                float h = (y2 - y1) / im_h;
    ...
    public static float[] labelToYoloV3NotFormat(Tensor label, int number, int im_w, int im_h) {
        ...
        for (int i = 0; i < number; i++) {
            for (int c = 0; c < channel; c++) {
                float clazz = label.getData()[i * channel * 5 + c * 5 + 0];
                float cx = label.getData()[i * channel * 5 + c * 5 + 1] / im_w;
                float cy = label.getData()[i * channel * 5 + c * 5 + 2] / im_h;
                float w = label.getData()[i * channel * 5 + c * 5 + 3] / im_w;
                float h = label.getData()[i * channel * 5 + c * 5 + 4] / im_h;
                if (cx > 0 && cy > 0 && w > 0 && h > 0) {
                    target[i * channel * 5 + c * 5 + 0] = cx;
                    target[i * channel * 5 + c * 5 + 1] = cy;
                    target[i * channel * 5 + c * 5 + 2] = w;
    ...
    public void transforms(Tensor input, Tensor label) {
         ...
         */
        getTransformsUtils(classNum, bboxSize).randomCropWithLabel(input, label, input.getShape()[2], input.getShape()[3], 0.2f);
        /**
         * 随机翻转
