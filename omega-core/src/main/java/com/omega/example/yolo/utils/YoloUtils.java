...
public class YoloUtils {
    ...
    public static YoloDetection[][] getYoloDetections(Tensor output, float[] anchors, int[] mask, int bbox_num, int outputs, int class_number, int orgW, int orgH, float thresh) {
        YoloDetection[][] dets = new YoloDetection[output.getShape()[0]][output.getShape()[2] * output.getShape()[3] * bbox_num];
        if (output.isHasGPU()) {
            output.syncHost();
        }
        int count = 0;
        for (int b = 0; b < output.getShape()[0]; b++) {
            for (int i = 0; i < output.getShape()[2] * output.getShape()[3]; i++) {
                ...
                for (int n = 0; n < bbox_num; n++) {
                    ...
                    int obj_index = entryIndex(b, output.getShape()[3], output.getShape()[2], n_index, 4, outputs, class_number);
                    float objectness = output.getData()[obj_index];
                    //		            if(objectness > thresh) {
                    nCount++;
                    //		            System.out.println(objectness);
                    int box_index = entryIndex(b, output.getShape()[3], output.getShape()[2], n_index, 0, outputs, class_number);
                    YoloDetection det = new YoloDetection(class_number);
                    det.setBbox(getYoloBox(output, anchors, mask[n], box_index, col, row, output.getShape()[3], output.getShape()[2], orgW, orgH, output.getShape()[2] * output.getShape()[3]));
                    det.setObjectness(objectness);
                    //		            det.setClasses(classes);
                    float classes = 0.0f;
                    float max = 0.0f;
                    for (int j = 0; j < class_number; ++j) {
                        int class_index = entryIndex(b, output.getShape()[3], output.getShape()[2], n_index, 4 + 1 + j, outputs, class_number);
                        if (output.getData()[class_index] >= max) {
                            max = output.getData()[class_index];
                            classes = j;
                        }
                        float prob = objectness * output.getData()[class_index];
        ...
        for (int b = 0; b < output.getShape()[0]; b++) {
            for (int i = 0; i < output.getShape()[2] * output.getShape()[3]; i++) {
                ...
                for (int n = 0; n < bbox_num; n++) {
                    ...
                    int box_index = entryIndex(b, output.getShape()[3], output.getShape()[2], n_index, 0, outputs, class_number);
                    YoloDetection det = new YoloDetection(class_number);
                    det.setBbox(getYoloBoxV7(output, anchors, mask[n], box_index, col, row, output.getShape()[3], output.getShape()[2], orgW, orgH, output.getShape()[2] * output.getShape()[3]));
                    det.setObjectness(objectness);
                    //		            det.setClasses(classes);
                    float classes = 0.0f;
                    float max = 0.0f;
