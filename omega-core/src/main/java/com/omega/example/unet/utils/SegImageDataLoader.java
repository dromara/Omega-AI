...
public class SegImageDataLoader extends BaseDataLoader {
    ...
    public void loadData(int pageIndex, int batchSize, Tensor input, Tensor label) {
        ...
        SegImageLoader.load(maskDirPath, extName, idxSet, indexs, label.getShape()[0], label, true, false);
        if (normalization) {
    ...
    public void loadData(int[] indexs, Tensor input, Tensor label) {
        ...
        SegGrayImageLoader.load(maskDirPath, extName, idxSet, indexs, input.getShape()[0], label, true, true);
        /**
         * copy data to gpu.
