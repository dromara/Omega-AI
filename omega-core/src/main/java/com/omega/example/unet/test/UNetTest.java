...
public class UNetTest {
    ...
    public static void testCell(String path, String labelPath, String filename, String extName, SegImageDataLoader dataLoader, int height, int width, SimpleUNet network, Tensor input, Tensor label) {
        ...
        for (int i = 0; i < output.getDataLength(); i++) {
            output.getData()[i] = output.getData()[i] * 255.0f;
        }
        String outImg = "H:\\unet-dataset\\cell\\" + filename + "." + extName;
        utils.createRGBImage(outImg, "png", ImageUtils.color2rgb2(output.getData(), output.getShape()[1], output.getShape()[2], output.getShape()[3], false), output.getShape()[2], output.getShape()[3], null, null);
        label.syncHost();
        for (int i = 0; i < label.getDataLength(); i++) {
            label.getData()[i] = label.getData()[i] * 255.0f;
        }
        String labelImg = "H:\\unet-dataset\\cell\\" + filename + "-mask" + "." + extName;
        utils.createRGBImage(labelImg, "png", ImageUtils.color2rgb2(label.getData(), label.getShape()[1], label.getShape()[2], label.getShape()[3], false), label.getShape()[2], label.getShape()[3], null, null);
    }
