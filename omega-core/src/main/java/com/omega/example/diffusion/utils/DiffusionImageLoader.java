    ...
    private void load() {
        for (int i = getStart(); i <= getEnd(); i++) {
            ...
            if (horizontalFilp) {
                ...
                data = DataTransforms.randomHorizontalFilp(data, newData, input.getShape()[1], input.getShape()[2], input.getShape()[3]);
            }
            for (int j = 0; j < data.length; j++) {
                data[j] = data[j] * getA()[i] + getNoise()[i * data.length + j] * getB()[i];
            }
            System.arraycopy(data, 0, getInput().getData(), i * data.length, data.length);
        }
    }
