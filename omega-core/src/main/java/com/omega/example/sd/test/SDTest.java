...
public class SDTest {
    public static void test_vqvae() {
        ...
        //		out.showDM();
        out.syncHost();
        out.setData( MatrixOperation.clampSelf(out.getData(), -1, 1));
        /**
         * print image
         *
         */
    ...
    public static void getVQVAE32_scale_factor() {
        ...
        float meanV = sum.syncHost()[0] / out.getDataLength() / indexs.length;
        ...
        double scale_factor = Math.sqrt(sum_cpu / out.getDataLength() / indexs.length);
    ...
    public static void loadLabels(String text, Tensor label, Tensor mask, BertTokenizer tokenizer, int maxContextLen) {
        ...
        for (int j = 0; j < maxContextLen; j++) {
            if (j < ids_n.length) {
                label.getData()[j] = ids_n[j];
                mask.getData()[j] = 0;
            } else {
                label.getData()[j] = 0;
                mask.getData()[j] = -10000.0f;
            }
        }
        mask.hostToDevice();
        label.hostToDevice();
    ...
    public static void loadLabels(String text, Tensor label, BPETokenizerEN tokenizer, int maxContextLen) {
        ...
        for (int j = 0; j < maxContextLen; j++) {
            if (j < ids.length) {
                label.getData()[0 * maxContextLen + j] = ids[j];
            } else {
                label.getData()[0 * maxContextLen + j] = 0;
            }
        }
        label.hostToDevice();
    }
