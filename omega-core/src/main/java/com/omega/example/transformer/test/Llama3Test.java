...
public class Llama3Test {
    ...
    public static void llama3_dp() {
        try {
            ...
            Tensor[] pos = RoPEKernel.getCosAndSin(input.getShape()[0], network.embedDim, network.headNum);
            for (int i = 0; i < gen_len; i++) {
                network.time = input.getShape()[0];
                //				System.out.println(input.number);
                //				input.showDM();
                String txt = genTxt(input, output, network, trainData, pre_txt.length(), pos);
                //				System.out.println("output txt="+txt);
                if (network.time > 1) {
                    pre_txt += txt.substring(input.getShape()[0] - 1, input.getShape()[0]);
                } else {
                    pre_txt += txt;
                }
        ...
        try {
            ...
            while (true) {
                ...
                for (int t = 0; t < max_len - startLen; t++) {
                    ...
                    RoPEKernel.getCosAndSin(input.getShape()[0], network.embedDim, network.headNum, pos);
                }
    ...
    public static void initParams(Tensor p, float mean, float std) {
        p.setData(RandomUtils.normal_(p.getDataLength(), mean, std));
    }
    ...
    public static Tensor createTxtData(Tensor input, String txt, int charDim, Map<Character, Integer> dictionary, int maxLenght) {
        ...
        if (input == null || input.getShape()[0] != charset.length) {
    ...
    public static int output2NextIDX(Tensor output, int nextTokenIdx) {
        if (nextTokenIdx < output.getShape()[0]) {
    ...
    public static int output2NextIDXTopN(Tensor output, int nextTokenIdx, int topK, CUDAManager cudaManager) {
        ...
        Tensor tmp = new Tensor(1, 1, 1, output.getShape()[3], true);
        Tensor prof = new Tensor(1, 1, 1, output.getShape()[3], true);
        if (nextTokenIdx < output.getShape()[0]) {
        ...
        if (meta != null) {
            ...
            if (dim == 1) {
                ...
                for (int n = 0; n < dataA.size(); n++) {
                    x.getData()[n] = dataA.get(n).floatValue();
                }
            } else if (dim == 2) {
                ...
                for (int n = 0; n < dataA.size(); n++) {
                    for (int w = 0; w < dataA.get(n).size(); w++) {
                        x.getData()[n * dataA.get(n).size() + w] = dataA.get(n).get(w).floatValue();
                    }
                }
            } else if (dim == 3) {
                float[][][] data = (float[][][]) meta;
                x.setData(MatrixUtils.transform(data));
            } else {
                ...
                for (int n = 0; n < N; n++) {
                    for (int c = 0; c < C; c++) {
                        for (int h = 0; h < H; h++) {
                            for (int w = 0; w < W; w++) {
                                x.getData()[n * x.getOnceSize() + c * H * W + h * W + w] = dataA.get(n).get(c).get(h).get(w).floatValue();
                            }
                        }
                    }
                }
                    ...
                    for (int c = 0; c < dataA.get(n).size(); c++) {
                        for (int h = 0; h < dataA.get(n).get(c).size(); h++) {
                            for (int w = 0; w < dataA.get(n).get(c).get(h).size(); w++) {
                                x.getData()[n * x.getOnceSize() + c * x.getShape()[2] * x.getShape()[3] + h * x.getShape()[3] + w] = dataA.get(n).get(c).get(h).get(w).floatValue();
                            }
                        }
                    }
                }
