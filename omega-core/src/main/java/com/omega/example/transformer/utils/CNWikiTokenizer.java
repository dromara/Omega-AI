    ...
    public void formatNotHeadToIdx(int b, int t, Integer[] onceToken, Tensor input, Tensor label) {
        if (tokenizer != null) {
            if (t + 1 < onceToken.length) {
                ...
                int curr = onceToken[t];
                input.getData()[b * max_len + t] = curr;
                label.getData()[(b * max_len + t) * vocab_size + tokenizer.eos] = 1.0f;
            } else {
                input.getData()[b * max_len + t] = tokenizer.unk;
                label.getData()[(b * max_len + t) * vocab_size + tokenizer.unk] = 1.0f;
            }
        } else {
            if (t + 1 < onceToken.length) {
                int curr = onceToken[t];
                ...
                int curr = onceToken[t];
                input.getData()[b * max_len + t] = curr;
                label.getData()[(b * max_len + t) * vocab_size + dictionary.get("<eos>")] = 1.0f;
            } else {
