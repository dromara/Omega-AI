...
public class CNWikiTokenizer2 extends BaseTokenizer {
        ...
        for (int n = 0; n < b; n++) {
            for (int hn = 0; hn < h; hn++) {
                for (int i = 0; i < size1; i++) {
                    for (int j = 0; j < size2; j++) {
                        if (i < j) {
                            mask.getData()[n * h * size1 * size2 + hn * size1 * size2 + i * size1 + j] = val;
                        }
                    }
                }
            }
    ...
    public static void triu(float val, int[] targetLens, Tensor mask) {
        for (int n = 0; n < mask.getShape()[0]; n++) {
            for (int hn = 0; hn < mask.getShape()[1]; hn++) {
                for (int i = 0; i < mask.getShape()[2]; i++) {
                    for (int j = 0; j < mask.getShape()[3]; j++) {
                        //						System.out.println(i+":"+targetLens[n]);
                        if (i < targetLens[n]) {
                            if (i < j) {
                                //								System.out.println(i+":"+j);
                                mask.getData()[n * mask.getShape()[1] * mask.getShape()[2] * mask.getShape()[3] + hn * mask.getShape()[2] * mask.getShape()[3] + i * mask.getShape()[2] + j] = val;
                            }
                        } else {
                            mask.getData()[n * mask.getShape()[1] * mask.getShape()[2] * mask.getShape()[3] + hn * mask.getShape()[2] * mask.getShape()[3] + i * mask.getShape()[2] + j] = val;
                        }
    ...
    public String decode(Tensor output) {
        int[] tokens = new int[output.getShape()[0]];
        for (int t = 0; t < output.getShape()[0]; t++) {
    ...
    public Tensor loadByTxtToIdx(String txt) {
        if (tokenizer != null) {
            ...
            //			testInput.clear();
            for (int t = 0; t < txt.length(); t++) {
                testInput.getData()[t] = dictionary.get(onceToken[t]);
            }
        }
        testInput.hostToDevice();
        return testInput;
    ...
    public void formatOnce(int t, String[] onceToken, Tensor input) {
        if (t == 0) {
            ...
            input.getData()[t * vocab_size + 1] = 1.0f;
            input.getData()[(t + 1) * vocab_size + dictionary.get(curr)] = 1.0f;
            return;
        }
        ...
        if ((t + 1) < onceToken.length) {
            ...
            input.getData()[(t + 1) * vocab_size + 0] = 1.0f;
        }
    }
    ...
    public void formatOnceNotHead(int t, String[] onceToken, Tensor input) {
        if ((t + 1) < onceToken.length) {
            ...
            input.getData()[t * vocab_size + dictionary.get(curr)] = 1.0f;
            ...
            input.getData()[t * vocab_size + 0] = 1.0f;
        }
    }
    ...
    public void formatNotHeadToIdx(int t, String[] onceToken, Tensor input) {
        if ((t + 1) < onceToken.length) {
            ...
            input.getData()[t] = dictionary.get(curr);
        } else if ((t + 1) == onceToken.length) {
            String curr = onceToken[t];
            if (sd.get(curr) != null) {
                curr = sd.get(curr);
            ...
            input.getData()[t] = dictionary.get("<pad>");
        }
    }
    ...
    public void format(int b, int t, Integer[] onceToken, Tensor input, Tensor label) {
        if (t == 0) {
            ...
            input.getData()[(b * max_len + t) * vocab_size + 1] = 1.0f;
            input.getData()[(b * max_len + t + 1) * vocab_size + curr] = 1.0f;
            label.getData()[(b * max_len + t) * vocab_size + next] = 1.0f;
            return;
        }
        if (t == onceToken.length - 1) {
            int curr = onceToken[t];
            input.getData()[(b * max_len + t + 1) * vocab_size + curr] = 1.0f;
            label.getData()[(b * max_len + t) * vocab_size + 2] = 1.0f;
            return;
        }
        if ((t + 1) < onceToken.length) {
            int curr = onceToken[t];
            ...
            input.getData()[(b * max_len + t) * vocab_size + 0] = 1.0f;
            label.getData()[(b * max_len + t) * vocab_size + 0] = 1.0f;
        }
    }
    ...
    public void formatNotHead(int b, int t, String[] onceToken, Tensor input, Tensor label) {
        if ((t + 1) == onceToken.length) {
            String curr = onceToken[t];
            input.getData()[(b * max_len + t) * vocab_size + dictionary.get(curr)] = 1.0f;
            label.getData()[(b * max_len + t) * vocab_size + dictionary.get("<sep>")] = 1.0f;
        } else {
            input.getData()[(b * max_len + t) * vocab_size + 0] = 1.0f;
            label.getData()[(b * max_len + t) * vocab_size + 0] = 1.0f;
        }
    ...
    public void formatNotHeadToIdx(int b, int t, Integer[] onceToken, Tensor input, Tensor label) {
        if (tokenizer != null) {
            if (t + 1 < onceToken.length) {
                int curr = onceToken[t];
                int next = onceToken[t + 1];
                input.getData()[b * max_len + t] = curr;
                label.getData()[b * max_len + t] = next;
                //				label.data[(b * max_len + t) * vocab_size + next] = 1.0f;
            } else if (t + 1 == onceToken.length) {
                int curr = onceToken[t];
                input.getData()[b * max_len + t] = curr;
                label.getData()[b * max_len + t] = tokenizer.eos;
                //				label.data[(b * max_len + t) * vocab_size + tokenizer.eos] = 1.0f;
            } else {
                input.getData()[b * max_len + t] = tokenizer.pad;
                label.getData()[b * max_len + t] = tokenizer.pad;
            ...
            if (t + 1 < onceToken.length) {
                ...
                int curr = onceToken[t];
                input.getData()[b * max_len + t] = curr;
                label.getData()[b * max_len + t] = dictionary.get("<eos>");
                //				label.data[(b * max_len + t) * vocab_size + dictionary.get("<eos>")] = 1.0f;
            } else {
                input.getData()[b * max_len + t] = dictionary.get("<pad>");
                label.getData()[b * max_len + t] = dictionary.get("<pad>");
