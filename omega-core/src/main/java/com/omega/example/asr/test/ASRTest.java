...
public class ASRTest {
	...
	public static void testASR() throws Exception {
		...
		float currentError = MatrixOperation.sum(loss.syncHost()) / label.getShape()[0];
		...
		currentError = MatrixOperation.sum(loss.syncHost()) / label.getShape()[0];
		System.out.println(currentError);
	}
	...
	public static void asr_predict() throws Exception {
		...
		while (true) {
			...
			input.setData(FBank.fbank(path, numBins, maxWavLength, 0, len));
			input.hostToDevice();
			feat_lens.getData()[0] = len[0];
