...
public class IndexDataLoader extends RNNDataLoader {
    ...
    public Tensor loadByTxt(String txt) {
        ...
        for (int t = 0; t < max_en; t++) {
            ...
            testInput.getData()[t * en_characters + valIdx] = 1.0f;
        }
    ...
    public void format(int b, int idx, Tensor enInput, Tensor deInput, Tensor label) {
        ...
        for (int t = 0; t < max_en; t++) {
            ...
            enInput.getData()[t * batchSize * en_characters + b * en_characters + valIdx] = 1.0f;
        }
        /**
         * 装载解码器数据
        ...
        for (int t = 0; t < max_ch - 1; t++) {
            ...
            deInput.getData()[t * batchSize * ch_characters + b * ch_characters + valIdx] = 1.0f;
        }
        for (int t = 0; t < max_ch - 1; t++) {
            int valIdx = ch_dictionary.get("<PAD>");
            if (t < ch_txt.length) {
            ...
            label.getData()[t * batchSize * ch_characters + b * ch_characters + valIdx] = 1.0f;
        }
    }
