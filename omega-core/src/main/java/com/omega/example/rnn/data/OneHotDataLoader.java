...
public class OneHotDataLoader extends RNNDataLoader {
    ...
    public void format(int b, int i, int t, Tensor input, Tensor label) {
        ...
        if (inputType == 1) {
            input.getData()[(t * batchSize + b)] = dictionary.get(curr);
            label.getData()[(t * batchSize + b) * characters + dictionary.get(next)] = 1.0f;
        } else {
            input.getData()[(t * batchSize + b) * characters + dictionary.get(curr)] = 1.0f;
            label.getData()[(t * batchSize + b) * characters + dictionary.get(next)] = 1.0f;
        }
    }
