...
//        Tensor V = new Tensor(batchSize, time, 1, headDim, MatrixUtils.order(len, 0.03f, 0.03f), true);
        ...
        Tensor delta = new Tensor(batchSize, time, 1, headDim, MatrixUtils.order(len, 0.1f, 0.1f), true);
    ...
    public void initFunction() {
        try {
            if (forward_function == null) {
                forward_function = getCudaManager().getLocalFunctionByModule("FlashAttentionKernel3.cu", "forward_attention_kernel");
            }
//            if (backward_function == null) {
//                backward_function = getCudaManager().getLocalFunctionByModule("FlashAttentionKernel3.cu", "flash_attention_2_backward_kernel");
//            }
        } catch (Exception e) {
    ...
    public void initParam(int B,int N) {
    	if(row_max == null || B != row_max.getShape()[0]) {
    ...
    public void forward(Tensor Q, Tensor K, Tensor V, Tensor output) {
        try {
        	...
        	int B = Q.getShape()[0];
            int N = Q.getShape()[1];
            int D = Q.getShape()[2] * Q.getShape()[3];
